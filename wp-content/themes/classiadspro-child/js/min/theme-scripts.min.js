...
pacz_double_menu();function pacz_page_title_intro(){"use strict";if(!is_touch_device()){$('#pacz-page-title').each(function(){var progressVal,currentPoint,$this=$(this),parentHeight=$this.outerHeight(),$fullHeight=$this.attr('data-fullHeight'),startPoint=0,endPoint=$this.offset().top+parentHeight,effectLayer=$this.find('.pacz-page-title-bg'),gradientLayer=$this.find('.pacz-effect-gradient-layer'),animation=$this.attr('data-intro');var layout=function(){var $heading=$this.find('.pacz-page-heading'),$fullHeight=$this.attr('data-fullHeight'),$header_height=0,$height=$this.attr('data-height'),page_title_full_height=0;if($.exists('#pacz-header.sticky-header')&&!$('#pacz-header').hasClass('transparent-header')){var $header_height=parseInt($('#pacz-header.sticky-header').attr('data-sticky-height'));}
...
$this.css('height',page_title_full_height);if($('#pacz-header').hasClass('transparent-header')&&$fullHeight=='true'){var header_height=parseInt($('#pacz-header').attr('data-height'));var padding=parseInt($this.css('padding-top'));$this.css({'padding':0});$heading.css({'padding-top':(page_title_full_height/2-$heading.height()/2)+'px'});}}
layout();if($fullHeight=='true'){$(window).on("debouncedresize",function(){layout();})}
if($('#pacz-header').hasClass('transparent-header')&&$fullHeight!='true'){var header_height=parseInt($('#pacz-header').attr('data-height'));var padding=parseInt($this.css('padding-top'));$this.css({'padding-top':(padding+header_height)+'px'})}
...
set();scrollAnimations.add(set);}});}}
var progressButton={loader:function(form){var $form=form,progressBar=$form.find(".pacz-progress-button .pacz-progress-inner"),buttonText=$form.find(".pacz-progress-button .pacz-progress-button-content"),progressButton=new TimelineLite();progressButton.to(progressBar,0,{width:"100%",scaleX:0,scaleY:1}).to(buttonText,.3,{y:-5}).to(progressBar,1.5,{scaleX:1,ease:Power2.easeInOut},"-=.1").to(buttonText,.3,{y:0}).to(progressBar,.3,{scaleY:0});},success:function(form){var $form=form,buttonText=$form.find(".pacz-button .pacz-progress-button-content, .pacz-contact-button .pacz-progress-button-content"),successIcon=$form.find(".pacz-progress-button .state-success"),progressButtonSuccess=new TimelineLite({onComplete:hideSuccessMessage});progressButtonSuccess.to(buttonText,.3,{paddingRight:20,ease:Power2.easeInOut},"+=1").to(successIcon,.3,{opacity:1}).to(successIcon,2,{opacity:1});function hideSuccessMessage(){progressButtonSuccess.reverse()}},error:function(form){var $form=form,buttonText=$form.find(".pacz-button .pacz-progress-button-content, .pacz-contact-button .pacz-progress-button-content"),errorIcon=$form.find(".pacz-progress-button .state-error"),progressButtonError=new TimelineLite({onComplete:hideErrorMessage});progressButtonError.to(buttonText,.3,{paddingRight:20},"+=1").to(errorIcon,.3,{opacity:1}).to(errorIcon,2,{opacity:1});function hideErrorMessage(){progressButtonError.reverse()}}};function sticky_header(){var $pacz_header=$('#pacz-header').first();var $pacz_body=$('body').first();if($pacz_header.hasClass('sticky-header')&&global_window_width>pacz_nav_res_width){var header_structure=$pacz_header.attr('data-header-structure');if(header_structure=='vertical'){var pacz_header_height=100;}else if(pacz_header_toolbar=true){var pacz_header_height=110;}else if(pacz_header_toolbar=false){var pacz_header_height=1;}
var chopScrollAnimation=function(){if(global_window_width>pacz_nav_res_width){if(scrollY>pacz_header_height){$pacz_header.addClass('sticky-trigger-header');}else{$pacz_header.removeClass('sticky-trigger-header');}}}
debouncedScrollAnimations.add(chopScrollAnimation);}}
function transparent_header_sticky(){var $pacz_header=$('#pacz-header');if($pacz_header.hasClass('transparent-header')&&global_window_width>pacz_nav_res_width){var trigger=false;var chopScrollAnimation=function(){var sharp_active=$('.pacz-sharp-slider.first-el-true').find('.swiper-slide-active').attr('data-header-skin'),header_structure=$pacz_header.attr('data-header-structure');if(header_structure=='vertical'){var pacz_header_height=100;}else{var pacz_header_height=parseInt((pacz_header_padding*2)+pacz_logo_height)+0;}
pacz_header_trans_offset=1;if(global_window_width>pacz_nav_res_width){if(scrollY>pacz_header_height||trigger){$pacz_header.addClass('header-offset-passed');}else{$pacz_header.removeClass('header-offset-passed');}
if(scrollY>pacz_header_trans_offset||trigger){$pacz_header.addClass('transparent-header-sticky sticky-trigger-header').removeClass('light-header-skin dark-header-skin');}else{if(sharp_active!=''&&typeof sharp_active!=='undefined'){$pacz_header.removeClass('transparent-header-sticky sticky-trigger-header').addClass(sharp_active+'-header-skin');}else{$pacz_header.removeClass('transparent-header-sticky sticky-trigger-header').addClass($pacz_header.attr('data-transparent-skin')+'-header-skin');}}}}
debouncedScrollAnimations.add(chopScrollAnimation);$('body').on('page_intro',function(){setTimeout(function(){trigger=true;chopScrollAnimation();},1000);});$('body').on('page_outro',function(){setTimeout(function(){trigger=false;chopScrollAnimation();},500);});}};function pacz_main_navigation_init(){"use strict";var $body=$('body');if(!$body.hasClass('navigation-initialised')){$(".main-navigation-ul").MegaMenu({type:"vertical",delay:200});$('#pacz-vm-menu').dlmenu();$body.addClass('navigation-initialised');}}
$('body').data('res-nav-appended',false);function pacz_main_navigation_functions(){if(global_window_width>pacz_nav_res_width){$('.pacz-responsive-nav').hide();setTimeout(function(){pacz_main_navigation_init();},200);if($('#pacz-header').attr('data-header-style')=='transparent'){$('#pacz-header').addClass('transparent-header '+$('#pacz-header').attr('data-transparent-skin')+'-header-skin');}}else{$('.main-navigation-ul, .pacz-vertical-menu').each(function(){var $this=$(this),res_nav=$('.responsive-nav-container');if(!res_nav.hasClass('res-nav-appended')){$this.clone().attr({"class":"pacz-responsive-nav"}).appendTo(res_nav);res_nav.addClass('res-nav-appended')}});$('.pacz-responsive-nav > li > ul').each(function(){if(!$(this).siblings('a').find('span').length){$(this).siblings('a').append('<span class="pacz-theme-icon-bottom-big pacz-nav-arrow pacz-nav-sub-closed"></span>');}});$('.pacz-responsive-nav').show();if(!$('body').data('res-nav-appended')){$('.pacz-responsive-nav > li').stop(true).on('click',function(e){var $this=$(this),$arrow=$this.find('.pacz-nav-arrow');if($(e.target).hasClass('pacz-nav-arrow')||$(e.target).hasClass('pacz-nav-open')){if($arrow.hasClass('pacz-nav-sub-closed')){$arrow.parent().siblings('ul').slideDown(450).end().end().removeClass('pacz-nav-sub-closed').addClass('pacz-nav-sub-opened');}else{$arrow.parent().siblings('ul').slideUp(450).end().end().removeClass('pacz-nav-sub-opened').addClass('pacz-nav-sub-closed');}
e.preventDefault();}});$('body').data('res-nav-appended',true);};if($('#pacz-header').attr('data-header-style')=='transparent'){$('#pacz-header').removeClass('transparent-header '+$('#pacz-header').attr('data-transparent-skin')+'-header-skin');}
if(!$.exists('#pacz-responsive-wrap')){var $header_height=0;var $window_height=$(window).outerHeight();if($.exists('#wpadminbar')){$header_height+=$('#wpadminbar').outerHeight();}
if($.exists('.responsive-nav-container')){$header_height+=$('#pacz-header').height();var nav_height=$window_height-$header_height;$('.responsive-nav-container').wrap('<div id="pacz-responsive-wrap" style="max-height:'+nav_height+'px; top:0"></div>');$('body').append('<div class="mobile-overlay"></div>');$('body').append('<div class="mobile-search-overlay"></div>');}
$(window).on('resize',function(){$('#pacz-responsive-wrap').css({'max-height':($(window).height()-$header_height)+'px'});});}
...
$('.pacz-animate-element').one('inview',function(event,visible){if(visible==true){$(this).addClass('pacz-in-viewport');}});});;function pacz_google_maps(){"use strict";$('.pacz-gmaps').each(function(){var $this=$(this),$id=$this.attr('id'),$zoom=parseInt($this.attr('data-zoom')),$latitude=$this.attr('data-latitude'),$longitude=$this.attr('data-longitude'),$address=$this.attr('data-address'),$latitude_2=$this.attr('data-latitude2'),$longitude_2=$this.attr('data-longitude2'),$address_2=$this.attr('data-address2'),$latitude_3=$this.attr('data-latitude3'),$longitude_3=$this.attr('data-longitude3'),$address_3=$this.attr('data-address3'),$pin_icon=$this.attr('data-pin-icon'),$pan_control=$this.attr('data-pan-control')==="true"?true:false,$map_type_control=$this.attr('data-map-type-control')==="true"?true:false,$scale_control=$this.attr('data-scale-control')==="true"?true:false,$draggable=$this.attr('data-draggable')==="true"?true:false,$zoom_control=$this.attr('data-zoom-control')==="true"?true:false,$modify_coloring=$this.attr('data-modify-coloring')==="true"?true:false,$saturation=$this.attr('data-saturation'),$hue=$this.attr('data-hue'),$gamma=$this.attr('data-gamma'),$lightness=$this.attr('data-lightness'),$fullHeight=$this.attr('data-fullHeight')==="true"?true:false,map_height,$header_height=0,$parent=$this.parent(),$height=$parent.height(),$styles;var mapDimensions=function(){if($.exists('#pacz-header')&&!$('#pacz-header').hasClass('transparent-header')){if($('#pacz-header').hasClass('sticky-header')){$header_height=parseInt($('#pacz-header').attr('data-sticky-height'));}else{$header_height=parseInt($('#pacz-header').attr('data-height'));}}
if($fullHeight===true){map_height=global_window_height-$header_height-global_admin_bar_height;}else{map_height=$height;}
$parent.height(map_height);if($parent.hasClass('pacz-gmaps-parallax')){$this.height(map_height+200);}else{$this.height(map_height);}}
if($modify_coloring==true){var $styles=[{stylers:[{hue:$hue},{saturation:$saturation},{lightness:$lightness},{gamma:$gamma},{featureType:"landscape.man_made",stylers:[{visibility:"on"}]}]}];}
...
$(window).load(function(){mapDimensions();setTimeout(function(){initialize();},500);});$(window).on('resize',function(){setTimeout(function(){mapDimensions();google.maps.event.trigger(map,'resize');},500);});});$(window).load(function(){if($.exists('.pacz-gmaps-parallax')){var pacz_skrollr=skrollr.init({forceHeight:false});pacz_skrollr.refresh($('.pacz-page-section'));}});};$(".pacz-header-search, .pacz-side-dashboard, .dashboard-trigger, .search-ajax-input, .pacz-quick-contact-inset").click(function(event){if(event.stopPropagation){event.stopPropagation();}else if(window.event){window.event.cancelBubble=true;}});$("html").click(function(){$('.header-search-icon').removeClass('search-clicked');$('form.header-searchform-input').fadeOut(250);$('#pacz-header').removeClass('header-search-triggered');$('.pacz-secondary-header').removeClass('header-search-triggered');$('.dashboard-trigger').removeClass('dashboard-active');$('.theme-main-wrapper, .pacz-side-dashboard').removeClass('dashboard-opened');});$('.header-search-icon').on('click',function(e){var $this=$(this);$this.parent().parent().parent().next().fadeIn(250);$this.parent().parent().parent().next().find('input[type=text]').focus();if(pacz_boxed_header=='0'){$this.parent().parent().parent().parent().addClass('header-search-triggered');}else{$this.parent().parent().parent().parent().parent().addClass('header-search-triggered');}
...
function pacz_causes(){$('.pacz-causes.grid-style .pacz-cause-item').each(function(){var $this=$(this),height=$this.outerHeight();$this.find('.team-info-wrapper').css({'height':height});});}
...
function pacz_tabs_responsive(){if($.exists('.pacz-tabs')){if(window.matchMedia('(max-width: 767px)').matches){$(".pacz-tabs").tabs("destroy");}
else{pacz_tabs();}}};function loop_audio_init(){if($.exists('.jp-jplayer')){$('.jp-jplayer.pacz-blog-audio').each(function(){var css_selector_ancestor="#"+$(this).siblings('.jp-audio').attr('id');var ogg_file,mp3_file,pacz_theme_js_path;ogg_file=$(this).attr('data-ogg');mp3_file=$(this).attr('data-mp3');$(this).jPlayer({ready:function(){$(this).jPlayer("setMedia",{mp3:mp3_file,ogg:ogg_file});},play:function(){$(this).jPlayer("pauseOthers");},swfPath:pacz_theme_js_path,supplied:"mp3, ogg",cssSelectorAncestor:css_selector_ancestor,wmode:"window"});});}}
...
pacz_flickr_feeds();;function pacz_fade_onload(){$(".pacz-mobile-image").fadeIn();$(".pacz-tablet-image").fadeIn();};function pacz_one_page_scroller(){$('.pacz-sharp-one-pager').each(function(){var $this=$(this),$tooltip_txt=[],$navigation=$this.attr('data-navigation')=="true"?true:false;$this.find('.section').each(function(){$tooltip_txt.push($(this).attr('data-title'));});var $header_height=0;if($.exists('#pacz-header.sticky-header')&&!$('#pacz-header').hasClass('transparent-header')){var $header_height=parseInt($('#pacz-header.sticky-header').attr('data-sticky-height'));}
...
$(document).ready(function(){pacz_go_to_top();pacz_main_navigation_functions();sticky_header();transparent_header_sticky();loop_audio_init();pacz_header_margin_style();pacz_social_share();pacz_google_maps();pacz_event_countdown();pacz_text_typer();pacz_one_page_scroller();pacz_one_pager_resposnive();pacz_child_ul_toggle_event();pacz_responsive_header_toolbar();pacz_header_checkout();$(window).load(function(){pacz_update_globals();pacz_fade_onload();pacz_lightbox_init();pacz_gallery_image();pacz_accordion();pacz_employees();pacz_causes();pacz_process_steps();pacz_gallery_thumbs_width();pacz_swipe_slider();pacz_hash_scroll();pacz_center_caption();loops_iosotop_init();isotop_load_fix();pacz_page_title_intro();pacz_secondary_header_res();pacz_tabs();secondary_header();pacz_button_animation();pacz_tabs_responsive();pacz_header_wpml();pacz_vertical_menu_submenu();pacz_theatre_autoplay_freeze();pacz_imagebox_autoplay_freeze();pacz_one_pager_resposnive();pacz_main_nav_scroll();section_to_full_height();});$(window).on("debouncedresize",function(){pacz_update_globals();section_to_full_height();pacz_main_navigation_functions();pacz_employees();pacz_causes();pacz_secondary_header_res();secondary_header();pacz_center_caption();pacz_one_pager_resposnive();transparent_header_sticky();sticky_header();pacz_tabs_responsive();pacz_responsive_header_toolbar();pacz_main_nav_scroll();});$('.pacz-go-top, .pacz-back-top-link').on('click',function(e){e.preventDefault();$("html, body").animate({scrollTop:0},800);});function pacz_go_to_top(){var pacz_go_top=$('.pacz-go-top');var pacz_quick_mail=$('.quick-button-container');var animationSet=function(){if(scrollY>700){pacz_go_top.removeClass('off').addClass('on');pacz_quick_mail.removeClass('go-right').addClass('go-left');}else{pacz_go_top.removeClass('on').addClass('off');pacz_quick_mail.removeClass('go-left').addClass('go-right');}}
debouncedScrollAnimations.add(animationSet);}
function pacz_love_post(){$('body').on('click','.pacz-love-this',function(){var $this=$(this),$id=$this.attr('id');if($this.hasClass('item-loved')){return false;}
if($this.hasClass('item-inactive')){return false;}
var $sentdata={action:'pacz_love_post',post_id:$id};$.post(ajaxurl,$sentdata,function(data){$this.find('span').html(data);$this.addClass('item-loved');});$this.addClass('item-inactive');return false;});}
pacz_love_post();function mobilecheck(){var check=false;(function(a){if(/(android|ipad|playbook|silk|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(a)||/1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(a.substr(0,4)))check=true})(navigator.userAgent||navigator.vendor||window.opera);return check;}
...
function pacz_header_checkout(){$('header').on('mouseenter','.pacz-shopping-cart',function(){$('.pacz-shopping-box').fadeIn(200).css({'z-index':100});});$('header').on('mouseleave','.pacz-shopping-cart',function(){$('.pacz-shopping-box').delay(500).fadeOut(200);});}
...
pacz_ajax_search();function pacz_contact_form(){if($.tools.validator!=undefined){$.tools.validator.addEffect("contact_form",function(errors){$.each(errors,function(index,error){var input=error.input;input.addClass('pacz-invalid');});},function(inputs){inputs.removeClass('pacz-invalid');});$(".captcha-change-image").on("click",function(e){e.preventDefault();changeCaptcha();});$(".captcha-form").each(function(){$(this).on("focus",function(){$(this).attr("placeholder",pacz_captcha_placeholder).removeClass('contact-captcha-invalid contact-captcha-valid');});});var changeCaptcha=function(){$(".captcha-image").attr("src",pacz_theme_dir+"/captcha/captcha.php?"+Math.random());}
var sendForm;var checkCaptcha=function(form,enteredCaptcha){$.get(pacz_theme_dir+"/captcha/captcha-check.php",{captcha:enteredCaptcha}).done(function(data){if(data!="ok"){changeCaptcha();form.find(".captcha-form").val("").addClass('contact-captcha-invalid').attr("placeholder",pacz_captcha_invalid_txt);}else{sendForm();changeCaptcha();form.find(".captcha-form").val("").addClass('contact-captcha-valid').attr("placeholder",pacz_captcha_correct_txt);}});}
$('.pacz-contact-form').validator({effect:'contact_form'}).submit(function(e){var form=$(this);if(!e.isDefaultPrevented()){var data={action:'pacz_contact_form',to:form.find('input[name="contact_to"]').val().replace("*","@"),name:form.find('input[name="contact_name"]').val(),phone:form.find('input[name="contact_phone"]').val(),email:form.find('input[name="contact_email"]').val(),content:form.find('textarea[name="contact_content"]').val()};sendForm=function(){progressButton.loader(form);$.post(ajaxurl,data,function(response){form.find('.pacz-contact-loading').fadeOut('slow');form.find('input').val("");form.find('textarea').val('');progressButton.success(form);});};var enteredCaptcha=form.find('input[name="captcha"]').val();if(form.find('.captcha-form').length){checkCaptcha(form,enteredCaptcha);}else{sendForm();}
e.preventDefault();}});}}
pacz_contact_form();$(this).find('.pacz-form-row input, .comment-form-row input, .pacz-login-form input').each(function(){$(this).focusin(function(){$(this).siblings('i').addClass('input-focused');});$(this).focusout(function(){$(this).siblings('i').removeClass('input-focused');});});$('form.pacz-login-form').each(function(){var $this=$(this);$this.on('submit',function(e){$('p.pacz-login-status',$this).show().text(ajax_login_object.loadingmessage);$.ajax({type:'POST',dataType:'json',url:ajax_login_object.ajaxurl,data:{'action':'ajaxlogin','username':$('#username',$this).val(),'password':$('#password',$this).val(),'security':$('#security',$this).val()},success:function(data){$('p.pacz-login-status',$this).text(data.message);if(data.loggedin===true){document.location.href=ajax_login_object.redirecturl;}}});e.preventDefault();});});var eventtype=mobilecheck()?'touchstart':'click';jQuery('.pacz-quick-contact-link').on(eventtype,function(){var $this=jQuery(this),$quickContact=jQuery('.pacz-quick-contact-overlay'),$quickContactInner=$quickContact.find('.pacz-quick-contact-inset');$quickContact.addClass('pacz-quick-contact-visible');return false;});jQuery('.pacz-quick-contact-overlay').on(eventtype,function(){$(this).removeClass('pacz-quick-contact-visible');});$('.box-close-btn').on(eventtype,function(){$(this).parent().fadeOut(300);return false;});function pacz_hash_scroll(){var headerHeight=classiadspro.modules.header.calcHeight();$(".pacz-smooth, .blog-comments").on('click',function(e){TweenLite.to(window,1.2,{scrollTo:{y:$($(this).attr("href")).offset().top-(headerHeight+2)},ease:Expo.easeInOut});e.preventDefault();});}
...
if($(window).width()<pacz_nav_res_width){headerHeight=0;}
...
d["top"===a.position?"prependTo":"appendTo"](c)}}};f.fn.fancybox=function(a){var d,e=f(this),c=this.selector||"",l=function(g){var h=f(this).blur(),k=d,l,m;g.ctrlKey||g.altKey||g.shiftKey||g.metaKey||h.is(".fancybox-wrap")||(l=a.groupAttr||"data-fancybox-group",m=h.attr(l),m||(l="rel",m=h.get(0)[l]),m&&""!==m&&"nofollow"!==m&&(h=c.length?f(c):e,h=h.filter("["+l+'="'+m+'"]'),k=h.index(this)),a.index=k,!1!==b.open(h,a)&&g.preventDefault())};a=a||{};d=a.index||0;c&&!1!==a.live?p.undelegate(c,"click.fb-start").delegate(c+":not('.fancybox-item, .fancybox-nav')","click.fb-start",l):e.unbind("click.fb-start").bind("click.fb-start",l);this.filter("[data-fancybox-start=1]").trigger("click");return this};p.ready(function(){var a,d;f.scrollbarWidth===w&&(f.scrollbarWidth=function(){var a=f('<div style="width:50px;height:50px;overflow:auto"><div/></div>').appendTo("body"),b=a.children(),b=b.innerWidth()-b.height(99).innerWidth();a.remove();return b});f.support.fixedPosition===w&&(f.support.fixedPosition=function(){var a=f('<div style="position:fixed;top:20px;"></div>').appendTo("body"),b=20===a[0].offsetTop||15===a[0].offsetTop;a.remove();return b}());f.extend(b.defaults,{scrollbarWidth:f.scrollbarWidth(),fixed:f.support.fixedPosition,parent:f("body")});a=f(s).width();K.addClass("fancybox-lock-test");d=f(s).width();K.removeClass("fancybox-lock-test");})})(window,document,jQuery);(function($){"use strict";var F=$.fancybox,format=function(url,rez,params){params=params||'';if($.type(params)==="object"){params=$.param(params,true);}
...
if(type){obj.href=url;obj.type=type;obj.autoHeight=false;}}};}(jQuery));jQuery(document).ready(function(){jQuery('.alsp-masonry-grid').masonry({itemSelector:'.grid-item',columnWidth:0,percentPosition:true,});jQuery(".header-hover-style-1 .current-menu-item a.menu-item-link").append('<div class="nav-hover-style1"><span></span><span></span><span></span></div>');jQuery('.pacz-header-mainnavbar li.cat-item:has(ul.children)').addClass('dropdown-submenu');jQuery('.pacz-header-mainnavbar ul.children').addClass('dropdown-menu');jQuery('.pacz-header-mainnavbar li.cat-item').on("mouseenter",function(e){jQuery(this).children('ul.children').show();e.stopPropagation();e.preventDefault();});jQuery('pacz-header-mainnavbar li.cat-item').on("mouseleave",function(e){jQuery(this).children('ul.children').hide();e.stopPropagation();e.preventDefault();});jQuery('.pacz-select2, .widget_archive select, .widget_categories select, .widget_text select').select2();});