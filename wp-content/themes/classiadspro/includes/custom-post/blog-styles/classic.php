...
if(is_sticky()){
	...
	/* Blog Meta */
        ...
        $output .= '<div class="blog-meta">';
			$output .= '<div class="blog-author">'.esc_html__('By:', 'classiadspro').'<span>'. get_the_author() .'</span></div>';
			$output .= '<time datetime="' . get_the_date() . '" itemprop="datePublished" pubdate>';
				$output .= esc_html__('Date:', 'classiadspro');
				$output .= '<a href="' . get_month_link(get_the_time("Y"), get_the_time("m")) . '">' . get_the_date() . '</a>';
			$output .= '</time>';
			$output .= '<div class="blog-categories">' .esc_html__('Categories:', 'classiadspro').'<span>'. get_the_category_list(', ') . '</span></div>';
...
$classic_excerpt = 'excerpt';
	...
	$output .= '<div class="blog-readmore-btn"><a class="classic-blog-link" title="'.get_the_title().'" href="'.get_permalink().'">'.esc_html__("Read More ","classiadspro").'<i class="pacz-icon-angle-double-right"></i></a></div>';
    $output .= '</article>';
