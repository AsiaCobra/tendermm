			...
			if(is_user_logged_in()){
				if(class_exists('alsp_dashboard_controller')){
					...
					echo '<div class="dropdown clearfix">';
						echo '<button class="dropbtn">'.$user_thumb .'<p class="author-name-header"><span class="author-displayname">'.$logedin_user_display_name.'</span><span class="author-nicename">'.$logedin_user_name.'</span></p></button>';
						echo '<div class="dropdown-content"><ul class="clearfix">';
							echo '<li><i class="pacz-icon-file-o"></i><a href="'.gdud_dashboard_uri().'">'.esc_html__('My Dashboard', 'classiadspro').'</a></li>';
							echo '<li><i class="pacz-icon-check-square-o"></i><a href="'.gdud_dashboard_uri().'">'.esc_html__('My Listings', 'classiadspro').'</a></li>';
							echo '<li><i class="pacz-li-settings"></i><a href="'.gdud_dashboard_uri().'?gdud_action=user-profile">'.esc_html__('Edit Profile', 'classiadspro').'</a></li>';
...
if ( !function_exists( 'add_first_nav_item' ) ) {
   ...
   <!-- </ul>
			...
			if($pacz_logo_location == 'header_section'  && $preset_headers != 12 && (!wp_is_mobile())) {
			ob_start();
			do_action( 'header_logo' );
			$output .= ob_get_contents();
			ob_end_clean();
