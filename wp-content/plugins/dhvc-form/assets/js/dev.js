...
!function( $ ) {
	...
	$(document).ready(function () {
		...
		$(document.body).on('dhvc_form_math_change', function(event, form) {
			...
			$('.paypal-total-value',$form).text(pp_total);
		});
		...
		$('.dhvc-form-datepicker[data-range_field]').each(function(){
			var $this = $(this),
				$range_field_name = $this.data('range_field'),
				$range_field = $('.dhvc-form-datepicker[data-field-name="' + $range_field_name + '"]');

			$range_field.xdsoftDatetimepicker('setOptions',{
				onChangeDateTime: function(){
					var nextDay = new Date($range_field.val());
					var nextDate = new Date(nextDay.getFullYear(), nextDay.getMonth(), nextDay.getDate() + parseInt($this.data('range_field_start_current')))
					if($this.data('range_field_set_value')==='yes'){
						var dateHelper = new DateFormatter();
						$this.val(dateHelper.formatDate(nextDate,dhvcformL10n.date_format)).attr('value',dateHelper.formatDate(nextDate,dhvcformL10n.date_format))
					}
