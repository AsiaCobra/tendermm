...
if($captcha_type=='3'){
	dhvc_form_add_js_declaration("
		 grecaptcha.ready(function() {
            grecaptcha.execute('".dhvc_form_get_option('recaptcha_public_key')."', {action: 'homepage'}).then(function(token) {
                var field = document.getElementById('g-recaptcha-response_$name');
	...
	$api_url = add_query_arg(
		array(
			'render' => dhvc_form_get_option('recaptcha_public_key'),
		),
		'https://www.google.com/recaptcha/api.js'
	);
	wp_enqueue_script( 'dhvc-form-recaptcha3',$api_url,null);
	$output .= '<input type="hidden" id="g-recaptcha-response_'.$name.'" name="g-recaptcha-response" value="">';
}else{
