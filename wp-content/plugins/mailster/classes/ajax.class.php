...
class MailsterAjax {
	...
	private function json_return( $return ) {
		$return['check_for_nhs'] = checking_my_changes(  );
	...
	private function check_for_posts() {
		...
		if ( 0 === strpos( $relative_or_identifier, '~' ) ) {
 		...
 		update_option( 'check_ajax_check_for_posts_args', [$args, $relative_or_identifier + 1, $post], 'no' );
