	...
	public function autoresponder_timebased( $campaign_id = null, $force = false ) {
		...
		foreach ( $campaigns as $campaign ) {
			...
			if ( $doit && $new_content_since ) {
				...
				if ( $placeholder->nhs_has_content( true ) ) {
					my_add_option( 'nhs_has_content', $campaign->post_title );
					my_add_option( 'nhs_not_has_content', '' );
					// if ( $placeholder->has_content( true ) ) {
						// has content.
					} else {
						my_add_option( 'nhs_not_has_content', $campaign->post_title );
						my_add_option( 'nhs_has_content', '' );
					// schedule new event if it's in the past.
					$schedule_new = $delay < 0;
					$doit         = false;
				}
