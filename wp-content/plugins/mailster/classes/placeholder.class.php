...
class MailsterPlaceholder {
	...
	public function __construct( $content = '', $deprecated = null ) {
		...
		update_option( 'checking', $content,'no' );
	...
	public function nhs_has_content( $check_for_modules = false ) {
		$html = trim( $this->get_content( true ) );
		if ( $check_for_modules ) {
			return preg_match( '/<module[^>]*?data-tag="(.*?)"/', $html );
		}
		return ! empty( $html );
	...
	private function remove_modules() {
		...
		if ( preg_match_all( '#<module[^>]*?data-tag="{(([a-z0-9_-]+):(-|~)?([\d]+)(;([0-9;,-]+))?)\}"(.*?)".*?</module>#ms', $this->content, $modules ) ) {
			...
			foreach ( $modules[0] as $i => $html ) {
				...
				$modules[$i]['search'] = $search;
				$modules[$i]['tag'] = $tag;
				$modules[$i]['post_type'] = $post_type;
				$modules[$i]['post_or_offset'] = $post_or_offset;
				$modules[$i]['type'] = $type;
				
				$cache_post = mailster_cache_delete( 'get_last_post' );
				$modules[$i]['cache_get_last_post'] = $cache_post;

				if ( empty( $type ) ) {
					$post = get_post( $post_or_offset );
					$modules[$i]['empty_type'] = $post;
				} else {
					$term_ids = ! empty( $modules[6][ $i ] ) ? explode( ';', trim( $modules[6][ $i ] ) ) : array();
					
					//NHS
					// $args = wp_parse_args( $this->last_post_args, [] );
					$real_term_ids          = $term_ids;
					if( implode_term_ids_and_meta_args( $term_ids, $post_type ) ){

						list( $term_ids, $args ) = implode_term_ids_and_meta_args( $term_ids, $post_type ); 
						// $args = wp_parse_args( $this->last_post_args, $args );
						// update_option( 'check_for_replace_dynamic', $this->last_post_args,'no' );
					}else{
						$this->set_last_post_args( ['suppress_filters' => 0 ] );
						$args = $this->last_post_args;
					}
					$modules[$i]['args'] = $args;
					$modules[$i]['offset'] = $post_or_offset;

					if ( '~' == $type ) {
						$post = mailster()->get_random_post( $post_or_offset, $post_type, $term_ids, $args, $this->campaignID, $this->subscriberID );
						// $post = mailster()->get_random_post( $post_or_offset, $post_type, $term_ids, $this->last_post_args, $this->campaignID, $this->subscriberID );
					} else {
						$post = mailster()->get_last_post( $post_or_offset -1 , $post_type, $term_ids, $args, $this->campaignID, $this->subscriberID );
				...
				$modules[$i]["post{$i}"] = '';
				if( $post ) {
					$modules[$i]["post{$i}"] = $post;
					$remove_content = false;
				} 

				$modules[$i]['remove_content'] = $remove_content;
				if ( ! $post ) {
					if( $remove_content ) $remove_content = true;
					// $this->content = '';
			...
			if( $remove_content ) $this->content = null;
			update_option( 'modules', $modules, 'no' );
	...
	private function replace_dynamic( $relative_to_absolute = false ) {
		...
		if ( $count = preg_match_all( '#\{(!)?((' . $pts . ')_([^}]+):(-|~)?([\d]+)(;([0-9;,-]+))?)\}#i', $this->content, $hits ) ) {
			...
			update_option( 'get_hits', $hits, 'no' );
