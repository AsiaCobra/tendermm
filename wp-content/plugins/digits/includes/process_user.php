...
function digits_process_form()
{
    ...
    $data = array();
    if (isset($_POST['digits_reg_mail']) || isset($_POST['digits_reg_username'])) {
...
function digits_process_forgotpassword()
{
    ...
    function recoverpassword($user_login)
    {
        ...
        return dig_wp_forgot_pass_mail($user_data, $key);
//TODO: need to update this code to make compatiable with digits forgot pass
    ...
    if ($digforgotpass == 1) {
        ...
        if ($password != $cpassword) {
            ...
            if ($userd != null) {
                do_action('password_reset', $userd, $password);
