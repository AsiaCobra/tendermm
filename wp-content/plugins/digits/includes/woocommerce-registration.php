...
{ ?>
    ...
    wp_enqueue_style('digits-login-style', plugins_url('digits/assets/css/login.min.css'), array(), null, 'all');

    digCountry();
    digits_add_style();
    digits_add_scripts(get_the_author_meta('digt_countrycode', $user->ID));
    wp_register_script('digits-login-script', plugins_url('digits/assets/js/login.js'), array('jquery'), null, true);
    ...
    if (get_option("dig_hide_countrycode", 0) == 1) {
        ?>
        <style>
...
if (!function_exists('wc_create_new_customer')) {
    ...
    function wc_create_new_customer($email, $username = '', $password = '', $args = array())
    {
        ...
        if (!empty($otp) || !empty($code) || dig_get_checkout_otp_verification() == 0) {
            ...
            } else if (dig_gatewayToUse($countrycode) == 1) {
...
function dig_update_user_profile($user_id)
{
    ...
    if (empty($p) && empty($_POST['email']) && !isValidEmail($_POST['email'])) {
        return;
    }
...
function wooc_extra_login()
{
    ...
    <input type="hidden" id="wc_code_dig" val="<?php if (isset($_POST['digt_countrycode']))
        echo sanitize_text_field($_POST['digt_countrycode']) ?>">
    <?php
...
function wc_dig_lost_pass()
{
    ...
    <div class="changePassword" style="display: none;">
        <p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
            <label for="reg_password"><?php _e('Password', 'digits'); ?> <span
