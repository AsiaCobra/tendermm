...
function dig_accountkit_dep_notice()
{
    ...
    if (isset($_POST['dig_hid_additional_gateways_dep_notice'])) {
        update_option('dig_additional_gateways_dep_notice', '1');

        return;
    }
    if (get_option('dig_accountkit_dep_notice', -1) != 1) {
        $dep_gateways = array('28' => 'Alfa Cell', '9' => 'Mobily.ws', '18' => 'Alibaba', '30' => 'Amazon Pinpoint', '25' => 'Amazon SNS', '33' => 'Alibaba (Go China)', '19' => 'ADNSMS', '22' => 'TargetSMS', '23' => 'Ghasedak', '24' => 'Farapayamak', '29' => 'IBulksms');

        $list = apply_filters('digits_addon', array());
        if (!in_array('additional-gateways', $list)) {
            $url = admin_url('admin.php?page=digits_settings&tab=addons');
            foreach ($dep_gateways as $dep_gateway => $gateway_name) {
                if (dig_is_gatewayEnabled($dep_gateway)) {
                    ?>
                    <div class="notice notice-error is-dismissible" style="display: block">
                        <p>
                            <b>UnitedOver Notice</b><br/>
                            <b><?php echo $gateway_name; ?></b> SMS gateway will be removed from main <b>Digits</b> and
                            <b>WPNotif</b> plugin
                            zip after 1st
                            March 2020 and will be added to Additional Gateways addon, so we request you to install the
                            free addon before 1st March 2020 for seamless migration.
                        ...
                        <form method="post">
                            <input type="hidden" name="dig_hid_additional_gateways_dep_notice"/>
                            <button type="submit" class="notice-dismiss" style="z-index: 99">
                                <span class="screen-reader-text">Dismiss this notice.</span>
                            </button>
                        </form>
...
add_filter('unitedover_sms_gateways', 'unitedover_custom_gateway_option', 100);
