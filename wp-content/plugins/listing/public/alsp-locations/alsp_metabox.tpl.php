	...
	<div id="alsp-locations-wrapper" class="form-horizontal">
		...
		if ($listing->locations)
			foreach ($listing->locations AS $location)
				alsp_renderTemplate('alsp-locations/alsp_metabox_child.tpl.php', array('listing' => $listing, 'location' => $location, 'locations_levels' => $locations_levels, 'delete_location_link' => (count($listing->locations) > 1) ? true : false));
		else
				alsp_renderTemplate('alsp-locations/alsp_metabox_child.tpl.php', array('listing' => $listing, 'location' => new alsp_location, 'locations_levels' => $locations_levels, 'delete_location_link' => false));
		?>
	</div>
