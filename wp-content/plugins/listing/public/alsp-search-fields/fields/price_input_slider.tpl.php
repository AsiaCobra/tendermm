...
<?php $min_value = (($search_field->min_max_value['min']) ? $search_field->content_field->formatPrice($search_field->min_max_value['min']) : __('min', 'ALSP')); ?>
<?php $max_value = (($search_field->min_max_value['max']) ? $search_field->content_field->formatPrice($search_field->min_max_value['max']) : __('max', 'ALSP')); ?>
...
?>
<div class="czprice-slider search-element-col alsp-field-search-block-<?php echo $search_field->content_field->id; ?> alsp-field-search-block-<?php echo $search_form_id; ?> alsp-field-search-block-<?php echo $index; ?> pull-left" style=" width:<?php echo $field_width ?>%; padding:0 <?php echo $gap_in_fields; ?>px;">
	...
	<script>
		(function($) {
			...
			$(function() {
				...
				var slider_params_<?php echo $index; ?> = ['<?php _e('min', 'ALSP'); ?>', <?php echo implode(',', $slider_params); ?>, '<?php _e('max', 'ALSP'); ?>'];
				...
				$('#range_slider_<?php echo $index; ?>').slider({
					...
					$.each(slider_params_<?php echo $index; ?>, function(index, value) {
						...
						if (index % odd_even_label == 0) {
							...
							var post_symbol_position = ' <?php echo esc_js($search_field->content_field->currency_symbol); ?>';
							<?php endif; ?>
							//var el = $('<label><span>|</span><span class="alsp-range-slider-label">' + pre_symbol_position + value + post_symbol_position + '</span></label>').css(position, (100/(slider_params_<?php echo $index; ?>.length-1))*index + '%');
						} else {
	...
	<div class="alsp-jquery-ui-slider">
		<div id="range_slider_<?php echo $index; ?>"></div>
		<div id="range_slider_<?php echo $index; ?>_scale" class="alsp-range-slider-scale"></div>
