			...
			<?php if ($public_control->do_initial_load): ?>
				...
				<div class="row alsp-listings-block-header">
					...
					<div class="alsp-found-listings">
						<?php printf(__('Found', "ALSP") . ' <span class="badge">%d</span>', $public_control->query->found_posts, _n($public_control->getListingsDirectory()->single, $public_control->getListingsDirectory()->plural, $public_control->query->found_posts)); ?>
					</div>
					<?php endif; ?>
				...
				<?php if ($public_control->listings): ?>
					...
					<div id="listing-block-<?php echo $public_control->hash; ?>" class="alsp-listings-block-content <?php echo $carousel; ?>  <?php if($public_control->args['scroll'] == 0){ echo $isotope_el_class; } ?> <?php if($public_control->args['scroller_nav_style'] == 2){ echo 'scroller_nav_style2'; } ?> clearfix" <?php if ($listing_style_to_show == 'show_grid_style'){ ?> style="margin-left:-<?php echo $grid_padding; ?>px; margin-right: -<?php echo $grid_padding; ?>px;" <?php } ?> <?php if($ALSP_ADIMN_SETTINGS['alsp_grid_masonry_display']){ ?> data-style="masonry" data-uniqid="<?php echo $public_control->hash; } ?>">
						<?php if ($public_control->args['scroll'] == 1): ?><!--cz custom -->
							<?php if(!empty($public_control->args['custom_category_link'])): ?>
								<div class="custom-category-link">
									<a href="<?php echo $public_control->args['custom_category_link']; ?>"><?php echo esc_html($public_control->args['custom_category_link_text']); ?></a>
								</div>
							<?php endif; ?>
					...
					<?php endif; ?>
				<?php else: ?>
					<?php printf(__('Found', "ALSP") . ' <span class="badge">%d</span> %s', $public_control->query->found_posts, _n($public_control->getListingsDirectory()->single, $public_control->getListingsDirectory()->plural, $public_control->query->found_posts)); ?>
				<?php endif; ?>
			</div>
			<?php endif; ?>
		</div>
