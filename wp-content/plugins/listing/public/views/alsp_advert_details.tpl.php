...
$id = uniqid();
	...
	if($ALSP_ADIMN_SETTINGS['alsp_single_listing_style'] == 2 ){ ?>
													...
													echo '<div class="alsp-custom-popup" data-popup="single_sharing_data">';
														echo '<div class="alsp-custom-popup-inner single-contact">';
															...
															echo '<div class="alsp-popup-content">';
																alsp_renderTemplate('views/aslp_share_call_ajax.tpl.php', array('post_id' => $listing->post->ID));
															echo'</div>';
														echo'</div>';
													echo'</div>';
												?>
				...
				<div id="<?php echo $listing->post->post_name; ?>" class="row" itemscope itemtype="http://schema.org/LocalBusiness">
					...
					<article id="post-<?php the_ID(); ?>" class="alsp-listing">
						...
						<div class="alsp-single-listing-text-content-wrap">
							...
							<?php alsp_renderTemplate('views/aslp_share_call_ajax.tpl.php', array('post_id' => $listing->post->ID)); ?>
							<?php endif; ?>
							...
							if($current_user->ID == $authorID) {
								...
								if($ALSP_ADIMN_SETTINGS['message_system'] == 'instant_messages'){
									if(is_user_logged_in()){
										...
										echo esc_html__('Please login to send message.', 'ALSP');
									}
									...
									if ($ALSP_ADIMN_SETTINGS['alsp_listing_contact_form'] && (!$listing->is_claimable || !$ALSP_ADIMN_SETTINGS['alsp_hide_claim_contact_form']) && ($listing_owner = get_userdata($listing->post->post_author)) && $listing_owner->user_email){
										...
										if (defined('WPCF7_VERSION') && alsp_get_wpml_dependent_option('alsp_listing_contact_form_7')){
											...
											alsp_renderTemplate('views/alsp_c_form.tpl.php', array('listing' => $listing)); 
					...
					echo '<div class="alsp-custom-popup-inner single-contact">';
						...
						echo '<div class="alsp-popup-content">';
							...
							if( is_user_logged_in()) {
								...
								echo esc_html__('Please login to send an offer.', 'ALSP');
							}
						echo'</div>';
					echo'</div>';
				echo'</div>';
					...
					echo '<div class="alsp-custom-popup-inner single-report">';
						...
						echo '<div class="alsp-popup-content">';
							...
							if( is_user_logged_in() && $current_user->ID == $authorID) {
								...
								alsp_renderTemplate('views/alsp_r_form.tpl.php', array('listing' => $listing));
							}
						echo'</div>';
					echo'</div>';
				echo'</div>';
					...
					<div class="modal-content">
					  ...
					  <button type="button" class="close" data-dismiss="modal">&times;</button>
						<?php if ($ALSP_ADIMN_SETTINGS['alsp_listing_contact_form'] && (!$listing->is_claimable || !$ALSP_ADIMN_SETTINGS['alsp_hide_claim_contact_form']) && ($listing_owner = get_userdata($listing->post->post_author)) && $listing_owner->user_email): ?>
							...
							<?php else: ?>
								<?php alsp_renderTemplate('views/alsp_c_form.tpl.php', array('listing' => $listing)); ?>
							<?php endif; ?>
							</div>
							<?php endif; ?>
					  </div>
		...
		?>
		<?php }elseif($ALSP_ADIMN_SETTINGS['alsp_single_listing_style'] == 4 ){ ?>
			<div class="single-listing alsp-content single-style-4">
			<?php alsp_renderMessages(); ?>
			...
			<?php while ($public_control->query->have_posts()): ?>
				...
				<div id="<?php echo $listing->post->post_name; ?>" itemscope itemtype="http://schema.org/LocalBusiness">
					...
					<article id="post-<?php the_ID(); ?>" class="alsp-listing">
						...
						<div class="listing-content-style4 clearfix">
							<?php if ($listing->title()): ?>
								<div class="row style-4-title-wrapper clearfix">
								...
								<div class="style-4-button-wrapper clearfix">
									<div class="style-4-action-btn price">
									<?php
										global $wpdb;
										$field_ids = $wpdb->get_results('SELECT id, type, slug FROM '.$wpdb->prefix.'alsp_content_fields');
										foreach( $field_ids as $field_id ) {
									...
									<div class="style-4-action-btn">
										<?php do_action('alsp_directory_frontpanel_claimButton', $buttons_view); ?>
										
									</div>
									<!--<div class="style-4-action-btn">
										<a class="pink" href="#">button</a>
									</div>
									<div class="style-4-action-btn">
										<a class="blue" href="#">button</a>
									</div>-->
								</div>
								<div class="divider"></div>
								<div class="style-4-author-wrapper clearfix">
									<div class="style-4-author-thumbnail pull-left">
										...
										<?php
											...
											$author_image_src_array = $author_avatar_url[0];
											if(!empty($author_image_src_array)) {
												$params = array( 'width' => 174, 'height' => 150, 'crop' => true );
										?>
												<img src="<?php echo bfi_thumb( $author_image_src_array, $params ); ?>"  alt="<?php echo get_the_author_meta('display_name', $authorID); ?>" />
										<?php
											} else { 
										?>
												<img src="<?php echo pacz_get_avatar_url ( get_the_author_meta('user_email', $authorID), $size = '90' ); ?>" alt="author" />
										<?php
											}
										?>
										</div>
										<?php
											if(!empty($phone_number) && $ALSP_ADIMN_SETTINGS['frontend_panel_user_phone']){
												$output_phone = '';
												$output_phone .= '<div class="author-phone style2">';
												...
												$numer_pre = '<span>'.esc_html__('Click To Show Number', 'pacz').'</span>'.$prephone_number.esc_html__('xxxxxx', 'pacz');
													$output_phone .= '<a id="number" data-pre="'.$numer_pre.'" data-last="'.$phone_number.'" class="" href="tel:'.$phone_number.'"><i class="pacz-fic-phone-call-3"></i><span class="number-main">'.$numer_pre.'</span></a>';
												$output_phone .= '</div>';
												echo $output_phone;
											}
										?>
									</div>
									<div class="style-4-author-content pull-left">
										<div class="style-4-author-content-top">
											<p class="author-reg-date"><?php echo esc_html__('Member since', 'pacz'). date_i18n( "M d, Y", strtotime( get_the_author_meta( 'user_registered', $authorID ) ) ); ?></p>
											<?php 
												$author_name = get_the_author_meta('display_name', $authorID);
												echo '<p class="author-name">'.$author_name.'</p>';
												if($ALSP_ADIMN_SETTINGS['author_profile_view']){
													echo '<div class="author-link"><a href="'.get_author_posts_url($authorID).'" class="">'. esc_html__('view all ads', 'pacz').'</a></div>'; 
												}
												echo '<div class="single-style-4-author-btns clearfix">';
														if($ALSP_ADIMN_SETTINGS['alsp_listing_contact']){ echo '<div class="author-btn-holder"><a class="" data-popup-open="single_contact_form" href="#">'.esc_html__('Contact', 'pacz').'</a></div>'; }
														if($ALSP_ADIMN_SETTINGS['alsp_listing_bidding']){ echo '<div class="author-btn-holder"><a data-popup-open="single_contact_form_bid" href="#" class="">'. esc_html__('Make An Offer', 'pacz').'</a></div>'; }
												echo '</div>';
											?>
										</div>
										<?php
										if($ALSP_ADIMN_SETTINGS['frontend_panel_social_links']){
												...
												$author_pinterest = get_the_author_meta('author_pinterest', $authorID);
												...
												if($author_fb || $author_tw || $author_linkedin || $author_pinterest || $author_ytube || $author_vimeo || $author_behance || $author_dribbble || $author_instagram || $author_flickr){
													echo '<div class="author-social-follow">';
														echo '<ul class="author-social-follow-ul">';
															echo '<li class="author-social-lable">'.esc_html('Follow Us', 'pacz').'</li>';
															if(!empty($author_fb)){
																echo '<li><a href="'.$author_fb.'" target_blank><i class="pacz-icon-facebook"></i></a></li>';
															}
															if(!empty($author_tw)){
																echo '<li><a href="'.$author_tw.'" target_blank><i class="pacz-icon-twitter"></i></a></li>';
															}
															if(!empty($author_pinterest)){
																echo '<li><a href="'.$author_pinterest.'" target_blank><i class="pacz-icon-pinterest"></i></a></li>';
															}
															if(!empty($author_linkedin)){
																echo '<li><a href="'.$author_linkedin.'" target_blank><i class="pacz-icon-linkedin"></i></a></li>';
															}
															if(!empty($author_ytube)){
																echo '<li><a href="'.$author_ytube.'" target_blank><i class="pacz-icon-youtube"></i></a></li>';
															}
															if(!empty($author_vimeo)){
																echo '<li><a href="'.$author_vimeo.'" target_blank><i class="pacz-icon-vimeo-square"></i></a></li>';
															}
															if(!empty($author_instagram)){
																echo '<li><a href="'.$author_instagram.'" target_blank><i class="pacz-icon-instagram"></i></a></li>';
															}
															if(!empty($author_flickr)){
																echo '<li><a href="'.$author_flickr.'" target_blank><i class="pacz-icon-flickr"></i></a></li>';
															}
															if(!empty($author_behance)){
																echo '<li><a href="'.$author_behance.'" target_blank><i class="pacz-icon-behance"></i></a></li>';
															}
															if(!empty($author_dribbble)){
																echo '<li><a href="'.$author_dribbble.'" target_blank><i class="pacz-icon-dribbble"></i></a></li>';
															}
														echo  '</ul>';
													echo  '</div>';
												}
			...
			//if($ALSP_ADIMN_SETTINGS['alsp_listing_bidding']){
				...
				echo '<div class="alsp-custom-popup" data-popup="single_shedule_form">';
					echo '<div class="alsp-custom-popup-inner single-shedule">';
						echo '<div class="alsp-popup-title">'.esc_html__('Shedule a Test Drive', 'ALSP').'<a class="alsp-custom-popup-close" data-popup-close="single_shedule_form" href="#"><i class="pacz-fic4-error"></i></a></div>';
						echo '<div class="alsp-popup-content">';
				...
				echo '<div class="alsp-custom-popup" data-popup="single_tradein_form">';
					echo '<div class="alsp-custom-popup-inner single-tradein">';
						echo '<div class="alsp-popup-title">'.esc_html__('Apply For TradeIn With Us', 'ALSP').'<a class="alsp-custom-popup-close" data-popup-close="single_tradein_form" href="#"><i class="pacz-fic4-error"></i></a></div>';
						echo '<div class="alsp-popup-content">';
							echo do_shortcode('[dhvc_form id="2578"]');
						echo'</div>';
					echo'</div>';
						...
						<div class="listing-main-content clearfix">
							<?php if ($listing->title()): ?>
								...
								<div class="listing-metas-single clearfix">
									...
									<div class="alsp-listing-date" datetime="<?php echo date("Y-m-d", mysql2date('U', $listing->post->post_date)); ?>T<?php echo date("H:i", mysql2date('U', $listing->post->post_date)); ?>"><i class="pacz-fic3-clock-circular-outline"></i><?php echo get_the_date(); ?></div>
									<div class="listing-views"><i class="pacz-fic3-medical"></i><?php echo sprintf(__('Views: %d', 'ALSP'), (get_post_meta($listing->post->ID, '_total_clicks', true) ? get_post_meta($listing->post->ID, '_total_clicks', true) : 0)); ?></div>
										<div class="listing-id"><i class="pacz-fic4-bookmark-white"></i><?php echo esc_html__('Tender ID', 'ALSP').': '.$listing->post->ID; ?></div>
								</div>
								<div class="single-listing-btns">
									<?php $hide_button_text = apply_filters('alsp_hide_button_text_on_listing', true)?>
									<?php $buttons_view = new alsp_buttons_view(array('hide_button_text' => $hide_button_text)); ?>
