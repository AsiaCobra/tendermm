...
<script>
	(function($) {
		...
		window.alsp_image_attachment_tpl_clogo = function(attachment_id, uploaded_file, title) {
			
			var image_attachment_tpl_clogo = '<div class="alsp-attached-item-clogo">' +
		...
		window.alsp_check_images_attachments_number_clogo = function() {
			if ($("#alsp-images-upload-wrapper.clogo .alsp-attached-item-clogo").length == 0) {
				
				<?php if (is_admin()): ?>
				$("#alsp-admin-upload-functions_clogo").show();
				<?php else: ?>
				$(".alsp-upload-item-clogo").show();
				<?php endif; ?>
			} else {
				<?php if (is_admin()): ?>
				$("#alsp-admin-upload-functions_clogo").hide();
				<?php else: ?>
				$(".alsp-upload-item-clogo").hide();
				<?php endif; ?>
			}
		}
		$(function() {
			alsp_check_images_attachments_number_clogo();

			$("#alsp-attached-images-wrapper.clogo").on("click", ".alsp-attached-item-delete", function() {
				$(this).parents(".alsp-attached-item-clogo").remove();
	
				alsp_check_images_attachments_number_clogo();
			});
			<?php if (!is_admin()): ?>
			$(document).on("click", ".alsp-upload-item-button-clogo", function(e){
				e.preventDefault();
			
				$(this).parent().find("input").click();
			});
			$('.alsp-upload-item-clogo').fileupload({
				...
				dropZone: $('.alsp-drop-attached-item.clogo'),
				add: function (e, data) {
					var jqXHR = data.submit();
				},
				send: function (e, data) {
					alsp_add_iloader_on_element($(this).find(".alsp-drop-attached-item.clogo"));
				},
				done: function(e, data) {
					var result = data.result;
					if (result.uploaded_file) {
						$(this).before(alsp_image_attachment_tpl_clogo(result.attachment_id, result.uploaded_file, data.files[0].name));
						alsp_custom_input_controls();
					} else {
						$(this).find(".alsp-drop-attached-item.clogo").append("<p>"+result.error_msg+"</p>");
					}
					$(this).find(".alsp-drop-zone.clogo").show();
					alsp_delete_iloader_from_element($(this).find(".alsp-drop-attached-item.clogo"));

					alsp_check_images_attachments_number_clogo();
					
					if ($('.alsp-attached-item-clogo').length != 0) {
						$('.alsp-upload-item-clogo').removeClass('full');
					}else{
						$('.alsp-upload-item-clogo').addClass('full');
					}
				}
			});
			<?php endif; ?>
...
<div id="alsp-images-upload-wrapper col-md-12" class="clogo alsp-content">
	<p class="alsp-submit-section-label alsp-submit-field-title"><?php _e('Company Logo', 'ALSP'); ?></p>
	<div id="alsp-attached-images-wrapper" class="clogo clearfix">
		...
		?>
			<div class="alsp-attached-item-clogo">
				
				<input type="hidden" name="attached_image_id[]" value="<?php echo $attachment_id; ?>" />
				<a href="<?php echo $src_full[0]; ?>" data-lightbox="listing_images" class="alsp-attached-item-img"><img src="<?php echo pacz_thumbnail_image_gen($image_src, 100, 70); ?>" width="250" height="250" alt="cover image" /></a>
				<div class="thumb-links clearfix">
				<div class="alsp-attached-item-delete fa fa-trash-o" title="<?php _e("delete", "ALSP"); ?>"></div>
			</div>
		</div>
		<?php if (!is_admin()): ?>
		<div class="alsp-upload-item-clogo full">
			<div class="alsp-drop-attached-item clogo">
				<div class="alsp-drop-zone clogo">
					<div class="dropzone-content">
						<span class="drophere"><?php _e("Drop here", "ALSP"); ?></span>
						<button class="alsp-upload-item-button-clogo btn btn-primary"><?php _e("Browse", "ALSP"); ?></button>
	...
	<script>
		(function($) {
			...
			$(function() {
				$('#alsp-admin-upload-image_clogo').click(function(event) {
					...
					frame.on('select', function() {
						...
						selection.each(function(attachment) {
							...
							if (attachment.type == 'image') {
								if ($("#alsp-attached-images-wrapper.clogo .alsp-attached-item-clogo").length == 0) {
									...
									$.ajax({
										...
										$("#alsp-attached-images-wrapper.clogo").append(alsp_image_attachment_tpl_clogo(this.attachment_id, this.attachment_url, this.attachment_title));
										alsp_check_images_attachments_number_clogo();
	...
	<div id="alsp-admin-upload-functions_clogo">
		<div class="alsp-upload-option">
			<input
				type="button"
				id="alsp-admin-upload-image_clogo"
...
<script>
	(function($) {
		...
		window.alsp_image_attachment_tpl_cover = function(attachment_id, uploaded_file, title) {
			
			var image_attachment_tpl_cover = '<div class="alsp-attached-item-cover">' +
				'<input type="hidden" name="attached_image_id[]" value="'+attachment_id+'" />' +
				'<a href="'+uploaded_file+'" data-lightbox="listing_images" class="alsp-attached-item-img">' +
				'<img src="'+uploaded_file+'" alt="" />' +
				'</a>' +
		...
		window.alsp_check_images_attachments_number_cover = function() {
			if ($("#alsp-images-upload-wrapper.cover .alsp-attached-item-cover").length == 0) {
				
				<?php if (is_admin()): ?>
				$("#alsp-admin-upload-functions_cover").show();
				<?php else: ?>
				$(".alsp-upload-item-cover").show();
				<?php endif; ?>
			} else {
				<?php if (is_admin()): ?>
				$("#alsp-admin-upload-functions_cover").hide();
				<?php else: ?>
				$(".alsp-upload-item-cover").hide();
				<?php endif; ?>
			}
		}
		$(function() {
			alsp_check_images_attachments_number_cover();

			$("#alsp-attached-images-wrapper.cover").on("click", ".alsp-attached-item-delete", function() {
				$(this).parents(".alsp-attached-item-cover").remove();
	
				alsp_check_images_attachments_number_cover();
			});
			<?php if (!is_admin()): ?>
			$(document).on("click", ".alsp-upload-item-button-cover", function(e){
				e.preventDefault();
			
				$(this).parent().find("input").click();
			});
			$('.alsp-upload-item-cover').fileupload({
				sequentialUploads: true,
				dataType: 'json',
				url: '<?php echo admin_url('admin-ajax.php?action=alsp_upload_image&post_id='.$listing->post->ID.'&_wpnonce='.wp_create_nonce('upload_images')); ?>',
				dropZone: $('.alsp-drop-attached-item.cover'),
				add: function (e, data) {
					var jqXHR = data.submit();
				},
				send: function (e, data) {
					alsp_add_iloader_on_element($(this).find(".alsp-drop-attached-item.cover"));
				},
				done: function(e, data) {
					var result = data.result;
					if (result.uploaded_file) {
						$(this).before(alsp_image_attachment_tpl_cover(result.attachment_id, result.uploaded_file, data.files[0].name));
						alsp_custom_input_controls();
					} else {
						$(this).find(".alsp-drop-attached-item.cover").append("<p>"+result.error_msg+"</p>");
					}
					$(this).find(".alsp-drop-zone.cover").show();
					alsp_delete_iloader_from_element($(this).find(".alsp-drop-attached-item.cover"));

					alsp_check_images_attachments_number_cover();
					
					if ($('.alsp-attached-item-cover').length != 0) {
						$('.alsp-upload-item-cover').removeClass('full');
					}else{
						$('.alsp-upload-item-cover').addClass('full');
					}
				}
			});
			<?php endif; ?>
...
<div id="alsp-images-upload-wrapper col-md-12" class="cover alsp-content">
	<p class="alsp-submit-section-label alsp-submit-field-title"><?php _e('Listing Cover image', 'ALSP'); ?></p>
	<div id="alsp-attached-images-wrapper" class="cover clearfix">
		...
		?>
			<div class="alsp-attached-item-cover">
		...
		<?php if (!is_admin()): ?>
		<div class="alsp-upload-item-cover full">
			<div class="alsp-drop-attached-item cover">
				<div class="alsp-drop-zone cover">
					<div class="dropzone-content">
						<span class="drophere"><?php _e("Drop here", "ALSP"); ?></span>
						<button class="alsp-upload-item-button-cover btn btn-primary"><?php _e("Browse", "ALSP"); ?></button>
	...
	<script>
		(function($) {
			...
			$(function() {
				$('#alsp-admin-upload-image_cover').click(function(event) {
					event.preventDefault();
					...
					frame.on('select', function() {
						...
						selection.each(function(attachment) {
							attachment = attachment.toJSON();
							if (attachment.type == 'image') {
								if ($("#alsp-attached-images-wrapper.cover .alsp-attached-item-clogo").length == 0) {
									alsp_ajax_loader_show();
									...
									$.ajax({
										...
										attachment_title: attachment.title,
										success: function (response_from_the_action_function){
										$("#alsp-attached-images-wrapper.cover").append(alsp_image_attachment_tpl_cover(this.attachment_id, this.attachment_url, this.attachment_title));
										alsp_check_images_attachments_number_cover();
	...
	<div id="alsp-admin-upload-functions_cover">
		<div class="alsp-upload-option">
			<input
				type="button"
				id="alsp-admin-upload-image_cover"
				class="btn btn-primary"
				value="<?php esc_attr_e('Upload image', 'ALSP'); ?>" />
		</div>
	</div>
...
<script>
	...
	(function($) {
		...
		window.alsp_check_images_attachments_number = function() {
			if (images_number > $("#alsp-images-upload-wrapper.gallery .alsp-attached-item").length) {
				<?php if (is_admin()): ?>
		...
		$(function() {
			...
			$("#alsp-attached-images-wrapper.gallery").on("click", ".alsp-attached-item-delete", function() {
...
<div id="alsp-images-upload-wrapper col-md-12" class="alsp-content">
	...
	<div id="alsp-attached-images-wrapper" class="gallery clearfix">
				...
				$('#alsp-admin-upload-image').click(function(event) {
					...
					frame.on('select', function() {
						...
						selection.each(function(attachment) {
							attachment = attachment.toJSON();
							if (attachment.type == 'image') {
								if (images_number > $("#alsp-attached-images-wrapper.gallery .alsp-attached-item").length) {
									alsp_ajax_loader_show();
									...
									$.ajax({
										...
										attachment_title: attachment.title,
										success: function (response_from_the_action_function){
										$("#alsp-attached-images-wrapper.gallery").append(alsp_image_attachment_tpl(this.attachment_id, this.attachment_url, this.attachment_title));
										alsp_check_images_attachments_number();
...
<div id="alsp-video-attach-wrapper" class="alsp-content">
	...
	<div id="alsp-attach-videos-functions">
		...
		<div class="alsp-upload-option">
			<input type="text" id="alsp-attach-video-input" class="form-control" placeholder="https://youtu.be/XXXXXXXXXXXX" />
		</div>
		<div class="alsp-upload-option">
			<input
				type="button"
