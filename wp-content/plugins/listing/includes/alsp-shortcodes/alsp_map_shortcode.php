...
class alsp_map_controller extends alsp_public_control {
	...
	public function init($args = array()) {
		...
		$shortcode_atts = array_merge(array(
				...
				'directories' => '',
				'num' => -1,
				'map_markers_is_limit' => !empty($args['custom_home']) ? (int)$ALSP_ADIMN_SETTINGS['alsp_map_markers_is_limit'] : 1, // How many map markers to display on the map (when listings shortcode is connected with map by unique string)
				'width' => '',
				'height' => !empty($args['custom_home']) ? (int)$ALSP_ADIMN_SETTINGS['alsp_default_map_height'] : 400,
				'radius_circle' => 0,
				'clusters' => !empty($args['custom_home']) ? (int)$ALSP_ADIMN_SETTINGS['alsp_enable_clusters'] : 0,
				'sticky_scroll' => 0,
				...
				'start_latitude' => '',
				'start_longitude' => '',
				'start_zoom' => !empty($args['custom_home']) ? (int)$ALSP_ADIMN_SETTINGS['alsp_default_map_zoom'] : 0,
				'map_style' => !empty($args['custom_home']) ? (int)alsp_getSelectedMapStyle() : 'default',
				'include_categories_children' => 0,
				'include_get_params' => 1,
				'search_on_map' =>  0,
				'search_on_map_open' => 0,
				//'show_categories_search' => !empty($args['custom_home']) ? (int)get_option('alsp_show_categories_search') : 1,
				//'categories_search_level' => !empty($args['custom_home']) ? (int)get_option('alsp_categories_search_nesting_level') : 1,
				'category' => 0,
				'exact_categories' => array(),
				//'show_keywords_search' => !empty($args['custom_home']) ? (int)get_option('alsp_show_keywords_search') : 1,
				//'keywords_ajax_search' => !empty($args['custom_home']) ? (int)get_option('alsp_keywords_ajax_search') : 1,
				'what_search' => '',
				//'show_radius_search' => !empty($args['custom_home']) ? (int)get_option('alsp_show_radius_search') : 1,
				//'radius' => !empty($args['custom_home']) ? (int)get_option('alsp_radius_search_default') : 0,
				//'show_locations_search' => !empty($args['custom_home']) ? (int)get_option('alsp_show_locations_search') : 1,
				//'locations_search_level' => !empty($args['custom_home']) ? (int)get_option('alsp_locations_search_nesting_level') : 1,
				//'show_address_search' => !empty($args['custom_home']) ? (int)get_option('alsp_show_address_search') : 1,
				'location' => 0,
				'exact_locations' => array(),
				'draw_panel' => !empty($args['custom_home']) ? (int)$ALSP_ADIMN_SETTINGS['alsp_enable_draw_panel'] : 0,
				'author' => 0,
				'enable_full_screen' => !empty($args['custom_home']) ? (int)$ALSP_ADIMN_SETTINGS['alsp_enable_full_screen'] : 1,
				'enable_wheel_zoom' => !empty($args['custom_home']) ? (int)$ALSP_ADIMN_SETTINGS['alsp_enable_wheel_zoom'] : 1,
				'enable_dragging_touchscreens' => !empty($args['custom_home']) ? (int)$ALSP_ADIMN_SETTINGS['alsp_enable_dragging_touchscreens'] : 1,
				'center_map_onclick' => !empty($args['custom_home']) ? (int)$ALSP_ADIMN_SETTINGS['alsp_center_map_onclick'] : 0,
				'categories' => '',
				'locations' => '',
				'related_directory' => 0,
...
");
		...
		$this->map = new alsp_maps($this->args, $this->request_by, $this->args['directories']);
		...
		apply_filters('alsp_map_controller_construct', $this);
	}
	...
	public function display() {
		...
		if ($this->args['custom_home'] || ($this->args['uid'] && $listings_controller = $alsp_instance->getListingsShortcodeByuID($this->args['uid']))) {
			if ($directory_controller = $alsp_instance->getShortcodeProperty('alsp-main')) {
				...
				if (!$listings_controller->map) {
					$listings_controller->map = new alsp_maps($this->args, $this->request_by, $this->args['directories']);
