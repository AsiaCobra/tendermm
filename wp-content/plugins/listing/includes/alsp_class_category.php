...
class alsp_categories_manager {
	...
	public function __construct() {
		...
		add_action(ALSP_CATEGORIES_TAX . '_add_form_fields', array ( $this, 'add_category_svg_image' ), 10, 2 );
		add_action( 'created_'. ALSP_CATEGORIES_TAX, array ( $this, 'save_category_svg_image' ), 10, 2 );
		add_action( 'edited_'. ALSP_CATEGORIES_TAX, array ( $this, 'update_save_category_svg_image' ));
		...
		add_action(ALSP_CATEGORIES_TAX . '_edit_form_fields', array ( $this, 'update_category_svg_image' ), 10, 2 );
	...
	public function taxonomy_columns($original_columns) {
		...
		$new_columns['alsp_category_icon'] = __('Icon ans Color', 'ALSP');
	...
	public function taxonomy_rows($row, $column_name, $term_id) {
		...
		if ($column_name == 'alsp_category_icon') {
			//return $row . $this->choose_icon_link($term_id).'<br/>'.$this->choose_icon_link2($term_id).'<br/>'.$this->choose_marker_icon_link($term_id);
	...
	public function select_icons_setting($term_id) {
		echo '<div class="dropdown show">';
			...
			echo '<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">';
				...
				echo '</div>';
				echo '<div class="dropmenu_item">';
					echo $this->choose_icon_link2($term_id);
				echo '</div>';
				echo '<div class="dropmenu_item">';
					echo $this->choose_marker_icon_link($term_id);
				echo '</div>';
				echo '<div class="dropmenu_item">';
					echo $this->choose_marker_icon_color($term_id);
				echo '</div>';
			echo '</div>';
		echo '</div>';
	}
	public function select_icon_form($term) {
		alsp_renderTemplate('alsp-categories/select_icon_form.tpl.php', array('term' => $term));
	}
	
	public function choose_icon_link($term_id) {
		$icon_file = $this->getCategoryIconFile($term_id);
		alsp_renderTemplate('alsp-categories/select_icon_link.tpl.php', array('term_id' => $term_id, 'icon_file' => $icon_file));
	}
	...
	public function select_category_icon_dialog() {
		...
		alsp_renderTemplate('alsp-categories/select_icons_dialog.tpl.php', array('categories_icons' => $categories_icons));
		die();
	}
	...
	public function select_icon_form2($term) {
		alsp_renderTemplate('alsp-categories/select_icon_form2.tpl.php', array('term' => $term));
	}
	public function choose_icon_link2($term_id) {
		$icon_file2 = $this->getCategoryIconFile2($term_id);
		alsp_renderTemplate('alsp-categories/select_icon_link2.tpl.php', array('term_id' => $term_id, 'icon_file2' => $icon_file2));
	}
	...
	public function select_category_icon_dialog2() {
		...
		alsp_renderTemplate('alsp-categories/select_icons_dialog2.tpl.php', array('categories_icons2' => $categories_icons2));
		die();
	}
	...
	public function select_marker_icon_form($term) {
		alsp_renderTemplate('alsp-categories/select_marker_icon_form.tpl.php', array('term' => $term));
	}
	public function choose_marker_icon_link($term_id) {
		$icon_name = $this->getCategoryMarkerIcon($term_id);
		alsp_renderTemplate('alsp-categories/select_marker_icon_link.tpl.php', array('term_id' => $term_id, 'icon_name' => $icon_name));
	}
	...
	public function select_marker_color_form($term) {
		alsp_renderTemplate('alsp-categories/select_marker_color_form.tpl.php', array('term' => $term));
	}
	public function choose_marker_icon_color($term_id) {
		$color = $this->getCategoryMarkerColor($term_id);
		alsp_renderTemplate('alsp-categories/select_marker_color_link.tpl.php', array('term_id' => $term_id, 'color' => $color));
	}
	...
	public function admin_enqueue_scripts_styles() {
		wp_enqueue_script('alsp_categories');
 ...
 */
 public function update_category_image ( $term, $taxonomy ) { ?>
   <tr class="form-field term-group-wrap">
	...
	<tr class="form-field term-group-wrap cat-svg-icon-meta">
	 <th scope="row">
       <label for="category-svg-icon-id"><?php _e( 'SVG ICON', 'ALSP' ); ?></label>
     </th>
     <td>
...
public function add_category_svg_image ( $term_id ) { ?>
   <div class="form-field term-group cat-svg-image-meta">
     <label for="category-svg-image-id"><?php _e('svg image ', 'ALSP'); ?></label>
     <input type="hidden" id="category-svg-image-id" name="category-svg-image-id" class="custom_media_url" value="">
     <div id="category-image-svg-wrapper" style="width:100px;max-height:100px;overflow:hidden;"></div>
     <p>
 ...
 public function save_category_svg_image ( $term_id, $tt_id ) {
   if( isset( $_POST['category-svg-image-id'] ) && '' !== $_POST['category-svg-image-id'] ){
     $image = $_POST['category-svg-image-id'];
     add_term_meta( $term_id, 'category-svg-image-id', $image, true );
   }
 }
 
 public function update_category_svg_image ( $term, $taxonomy ) { ?>
   <tr class="form-field term-group-wrap cat-image-meta">
     <th scope="row">
       <label for="category-svg-image-id"><?php _e( 'SVG IMAGE ', 'ALSP' ); ?></label>
     </th>
     <td>
       <?php $image_id = get_term_meta ( $term -> term_id, 'category-svg-image-id', true ); ?>
       <input type="hidden" id="category-svg-image-id" name="category-svg-image-id" value="<?php echo $image_id; ?>">
       <div id="category-image-svg-wrapper" style="width:100px;max-height:100px;overflow:hidden;">
         <?php if ( $image_id ) { ?>
           <?php echo wp_get_attachment_image ( $image_id, 'full' ); ?>
         <?php } ?>
       </div>
 ...
 public function update_save_category_svg_image ( $term_id) {
   if( isset( $_POST['category-svg-image-id'] ) && '' !== $_POST['category-svg-image-id'] ){
     $image = $_POST['category-svg-image-id'];
     update_term_meta ( $term_id, 'category-svg-image-id', $image );
	 
   } else {
     update_term_meta ( $term_id, 'category-svg-image-id', '' );
   }
 }
   ...
   <script>
	 ...
	 function alsp_svg_media_upload(button_class) {
         ...
         $('body').on('click', button_class, function(e) {
           ...
           wp.media.editor.send.attachment = function(props, attachment){
             if ( _custom_media ) {
               $('#category-svg-image-id').val(attachment.id);
               $('#category-image-svg-wrapper').html('<img class="custom_media_image" src="" style="margin:0;padding:0;max-height:100px;float:none;" />');
               $('#category-image-svg-wrapper .custom_media_image').attr('src',attachment.sizes.thumbnail.url).css('display','block');
             } else {
               return _orig_send_attachment.apply( button_id, [props, attachment] );
             }
            }
     ...
     $(document).ajaxComplete(function(event, xhr, settings) {
       ...
       if( $.inArray('action=add-tag', queryStringArr) !== -1 ){
         ...
         if($response!=""){
           // Clear the thumb image
           $('#category-image-svg-wrapper').html('');
         }
       }
     });
	 // svg image end
