...
class alsp_settings_manager {
	...
	public function plugin_settings() {
		global $alsp_instance, $alsp_social_services, $alsp_google_maps_styles, $sitepress;

		if ($alsp_instance->index_page_id === 0 && isset($_GET['action']) && $_GET['action'] == 'directory_page_installation') {
			$page = array('post_status' => 'publish', 'post_title' => __('ALSP Listing', 'ALSP'), 'post_type' => 'page', 'post_content' => '[alsp]', 'comment_status' => 'closed');
			if (wp_insert_post($page))
				alsp_addMessage(__('"ALSP Listing" page with [alsp] shortcode was successfully created, thank you!'));
		}
