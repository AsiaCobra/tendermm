...
abstract class alsp_widget extends WP_Widget {
	...
	public function __construct($id_base, $name, $description = '') {
		...
		$this->addField("textfield", "title", __("Title", "ALSP"));
	...
	public function wp_enqueue_scripts() {
		...
		if (isset($widget_options_all[$this->number])) {
			...
			if (is_active_widget(false, false, $this->id_base, true) && (empty($current_widget_options['visibility']) || !empty($alsp_instance->public_controls))) {
	...
	public function wp_enqueue_custom_style() {
		...
		if (isset($widget_options_all[$this->number])) {
			$current_widget_options = $widget_options_all[$this->number];
			if (is_active_widget(false, false, $this->id_base, true) && (empty($current_widget_options['visibility']) || !empty($alsp_instance->public_controls))) {
				//$alsp_instance->enqueue_scripts_styles_custom(true);
			}
		}
	}
	...
	public function renderDescription($field){
		if (!empty($field->description)) {
			echo "<span class=\"alsp-widget-description\">" . esc_html($field->description) . "</span>";
		}
	}

	public function renderDependency($field){
		if ($field->dependency) {
			echo "<span class=\"alsp-widget-dependency\" data-dependency-element=\"{$field->dependency['element']}\" data-dependency-value=\"{$field->dependency['value']}\"></span>";
		}
	}
					...
					$(function() {
							...
							inputs.each(function() {
								$(this).on("change", function() {
									var input = $(this);
									var dependencies = input.parents(".widget-content, .so-content").find(".alsp-widget-dependency");
	...
	public function render_dropdown_field($field, $instance) {
		...
		$this->renderDependency($field);
		$this->renderLabel($field, $id);
		echo "<select class=\"widefat\" id=\"{$id}\" name=\"{$name}\" class=\"widefat alsp-widget-select-control\" data-original-name=\"{$field->name}\">";
	...
	public function render_mapstyle_field($field, $instance) {
		...
		echo "<select id=\"{$id}\" name=\"{$name}\" class=\"widefat alsp-widget-select-control\">";
		echo "<option value=\"0\" " . selected($value, 0) . ">" . __('Default', 'ALSP') . "</option>";
		foreach (alsp_getAllMapStyles() AS $style_name=>$style) {
	...
	public function render_ordering_field($field, $instance) {
		...
		echo "<select id=\"{$id}\" name=\"{$name}\" class=\"widefat alsp-widget-select-control\">";
		echo "<option value=\"\" " . selected($value, '', false) . ">" . __('Default', 'ALSP') . "</option>";
	...
	public function render_level_field($field, $instance, $multiple = false) {
		...
		echo "<select id=\"{$name}" . ($multiple ? "_select" : "") . "\" name=\"{$name}" . ($multiple ? "_select" : "") . "\" " . ($multiple ? "multiple=\"multiple\" class=\"widefat alsp-widget-select-multiple-control\"" : "class=\"widefat alsp-widget-select-control\"") . " data-original-name=\"{$field->name}\">";
		echo "<option value=\"0\" " . selected($value, 0) . ">" . __('- All levels -', 'ALSP') . "</option>";
	...
	public function render_directory_field($field, $instance, $multiple = false) {
		...
		if ($alsp_instance->directories->isMultiDirectory()) {
			...
			$this->renderDependency($field);
			$this->renderLabel($field, $id);
			echo "<select id=\"{$name}" . ($multiple ? "_select" : "") . "\" name=\"{$name}" . ($multiple ? "_select" : "") . "\" " . ($multiple ? "multiple=\"multiple\" class=\"widefat alsp-widget-select-multiple-control\"" : "class=\"widefat alsp-widget-select-control\"") . ">";
			echo "<option value=\"0\" " . selected($value, 0) . ">" . __('- Auto -', 'ALSP') . "</option>";
	...
	public function render_categoryfield_field($field, $instance, $multiple = false) {
		...
		$this->renderLabel($field, $id);
		echo "<select id=\"{$name}" . ($multiple ? "_select" : "") . "\" name=\"{$name}" . ($multiple ? "_select" : "") . "\" " . ($multiple ? "multiple=\"multiple\" class=\"widefat alsp-widget-select-multiple-control\"" : "class=\"widefat alsp-widget-select-control\"") . ">";
		echo "<option value=\"\" " . selected($value, '') . ">" . __('- No category selected -', 'ALSP') . "</option>";
		ob_start();
	...
	public function render_locationfield_field($field, $instance, $multiple = false) {
		...
		$this->renderLabel($field, $id);
		echo "<select id=\"{$name}" . ($multiple ? "_select" : "") . "\" name=\"{$name}" . ($multiple ? "_select" : "") . "\" " . ($multiple ? "multiple=\"multiple\" class=\"widefat alsp-widget-select-multiple-control\"" : "class=\"widefat alsp-widget-select-control\"") . ">";
		echo "<option value=\"\" " . selected($value, '') . ">" . __('- No location selected -', 'ALSP') . "</option>";
		ob_start();
	...
	public function render_contentfields_field($field, $instance) {
		...
		$this->renderDependency($field);
		$this->renderLabel($field, $id);
		echo "<select id=\"{$name}_select\" name=\"{$name}_select\" multiple=\"multiple\" class=\"widefat alsp-widget-select-multiple-control\">";
		echo "<option value=\"\" " . selected($value, '') . ">" . __('- All content fields -', 'ALSP') . "</option>";
		echo "<option value=\"-1\" " . selected($value, -1) . ">" . __('- No content fields -', 'ALSP') . "</option>";
	...
	public function outputJSforMultipleDropBox() {
		$out = '<script>
				$("body").on("change", ".alsp-widget-select-multiple-control", function() {
	...
	public function render_colorpicker_field($field, $instance) {
		...
		echo '<script>
				(function($) {
					"use strict";
					$(function() {
						$(".alsp-select-color").wpColorPicker({
		...
		echo "<input
				...
				name=\"{$name}\"
				value=\"{$value}\"
				class=\"alsp-select-color widefat\"
			/>";
		$this->renderDescription($field, $id);
		echo "</p>";
	}
	...
	public function render_datefieldmin_field($field, $instance) {
		...
		$this->renderDependency($field);
		$this->renderLabel($field, $id);
		alsp_renderTemplate('alsp-search-fields/fields/datetime_input_vc_min.tpl.php', array('settings' => $settings, 'value' => $value, 'dateformat' => alsp_getDatePickerFormat()));
		$this->renderDescription($field, $id);
		echo "</p>";
	}
	...
	public function render_datefieldmax_field($field, $instance) {
		...
		$this->renderDependency($field);
		$this->renderLabel($field, $id);
		alsp_renderTemplate('alsp-search-fields/fields/datetime_input_vc_max.tpl.php', array('settings' => $settings, 'value' => $value, 'dateformat' => alsp_getDatePickerFormat()));
		$this->renderDescription($field, $id);
		echo "</p>";
	}
...
add_action('widgets_init', 'alsp_register_widgets');
function alsp_register_widgets() {
	foreach (get_declared_classes() as $class) {
		if (is_subclass_of($class, "alsp_widget")) {
