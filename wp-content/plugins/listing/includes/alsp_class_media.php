...
class alsp_media_manager {
	...
	public function mediaMetabox() {
		...
		alsp_renderTemplate('views/alsp_m_metabox.tpl.php', array('listing' => $listing));
	}
	...
	public function upload_image() {
		...
		if (wp_verify_nonce($_GET['_wpnonce'], 'upload_images') && isset($_FILES['browse_file']) && ($_FILES['browse_file']['size'] > 0) && isset($_GET['post_id']) && ($post_id = $_GET['post_id'])) {
			if (($listing = alsp_getListing($post_id))) {
				if (alsp_current_user_can_edit_advert($post_id) || !$listing->post->post_author) {
			...
			if (($listing = alsp_getListing($post_id))) {
				...
				$result['error_msg'] = __('Wrong listing ID.', 'ALSP');
		}elseif (wp_verify_nonce($_GET['_wpnonce'], 'upload_images') && isset($_FILES['browse_file_cover']) && ($_FILES['browse_file_cover']['size'] > 0) && isset($_GET['post_id']) && ($post_id = $_GET['post_id'])) {
			if (($listing = alsp_getListing($post_id))) {
				if (alsp_current_user_can_edit_advert($post_id) || !$listing->post->post_author) {
					...
					//if ($listing->level->images_number > $existed_images_count) {
						// Get the type of the uploaded file. This is returned as "type/extension"
						$arr_file_type = wp_check_filetype(basename($_FILES['browse_file_cover']['name']));
						$uploaded_file_type = $arr_file_type['type'];
						...
						if (in_array($uploaded_file_type, $allowed_file_types)) {
							...
							// Handle the upload using WP's wp_handle_upload function. Takes the posted file and an options array
							$uploaded_file = wp_handle_upload($_FILES['browse_file_cover'], $upload_overrides);
	...
	public function upload_media_image() {
		if (wp_verify_nonce($_POST['_wpnonce'], 'upload_images') && isset($_POST['attachment_id']) && isset($_POST['post_id']) && ($post_id = $_POST['post_id'])) {
			if (($listing = alsp_getListing($post_id))) {
				if (alsp_current_user_can_edit_advert($post_id)) {
					// insert attachment ID to the post meta
	...
	public function admin_enqueue_scripts_styles() {
		...
		if (is_admin() && current_user_can('upload_files')) {
			...
			wp_register_script('alsp_fileupload', ALSP_RESOURCES_URL . 'js/min/jquery.fileupload.min.js', array('jquery'), false, true);
			wp_register_script('alsp_fileupload_iframe', ALSP_RESOURCES_URL . 'js/min/jquery.iframe-transport.min.js', array('jquery'), false, true);
