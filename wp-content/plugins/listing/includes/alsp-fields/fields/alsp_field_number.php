...
class alsp_content_field_number extends alsp_content_field {
	...
	public function configure() {
		...
		if (alsp_getValue($_POST, 'submit') && wp_verify_nonce($_POST['alsp_configure_content_fields_nonce'], ALSP_PATH)) {
			...
			if ($validation->run()) {
				...
				alsp_renderTemplate('alsp_fields/fields/alsp_number_configuration.tpl.php', array('content_field' => $this));
			}
		} else
			alsp_renderTemplate('alsp_fields/fields/alsp_number_configuration.tpl.php', array('content_field' => $this));
	}
	...
	public function renderInput() {
		if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_number_input_'.$this->id.'.tpl.php'))) {
			$template = 'alsp_fields/fields/alsp_number_input.tpl.php';
		}
	...
	public function renderOutput($listing = null) {
		if (is_numeric($this->value)) {
			...
			if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_number_output_'.$this->id.'.tpl.php'))) {
				$template = 'alsp_fields/fields/alsp_number_output.tpl.php';
			}
			
			$template = apply_filters('alsp_content_field_output_template', $template, $this, $listing);
			
			alsp_renderTemplate($template, array('content_field' => $this, 'formatted_number' => $formatted_number, 'listing' => $listing));
		}
	}
