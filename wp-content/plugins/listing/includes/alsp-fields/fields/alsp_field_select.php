...
class alsp_content_field_select extends alsp_content_field {
	...
	public function configure() {
		...
		if (alsp_getValue($_POST, 'submit') && wp_verify_nonce($_POST['alsp_configure_content_fields_nonce'], ALSP_PATH)) {
			...
			if ($validation->run()) {
				...
				alsp_renderTemplate('alsp_fields/fields/alsp_select_configuration.tpl.php', array('content_field' => $this));
			}
		} else {
			alsp_renderTemplate('alsp_fields/fields/alsp_select_configuration.tpl.php', array('content_field' => $this));
		}
	...
	public function renderInput() {
		if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_select_input_'.$this->id.'.tpl.php'))) {
			$template = 'alsp_fields/fields/alsp_select_input.tpl.php';
		}
	...
	public function validateValues(&$errors, $data) {
		...
		elseif( $this->id == 92 ){
			$selected_items_array = $validation->result_array($field_index);
			...
			$selected_item = $validation->result_array($field_index);
			if (!in_array($selected_item, array_keys($this->selection_items)))
				$errors[] = sprintf(__("This selection option index \"%d\" doesn't exist", 'ALSP'), $selected_item);
		...
		// 	}else{
		// 		if (!in_array($selected_items_array, array_keys($this->selection_items)))
		// 			$errors[] = sprintf(__("This selection option index \"%d\" doesn't exist", 'ALSP'), $selected_items_array);
		// 	}
	...
	public function saveValue($post_id, $validation_results) {
		...
		// if( $this->id == 92  ) update_post_meta($post_id, 'tender_feild', $validation_results);
	...
	public function loadValue($post_id) {
		if( $this->id == 92  ){
			...
			// $this->value = get_post_meta($post_id, '_content_field_' . $this->id);
			// $result = array();
			// foreach ($this->selection_items AS $key=>$value) {
			// 	if (array_search($key, $this->value) !== FALSE){

			// 		$result[] = $key;
			// 	}elseif( in_array( $key, $this->value ) ){
			// 		$result[] = $value;
			// 		$this->value=$value;
			// 	}
			// }
	...
	public function renderOutput($listing = null) {
		if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_select_radio_output_'.$this->id.'.tpl.php'))) {
			$template = 'alsp_fields/fields/alsp_select_radio_output.tpl.php';
		}
	...
	public function content_fields_options_into_strings() {
		...
		if (function_exists('wpml_object_id_filter') && $sitepress) {
			foreach ($this->selection_items AS $key=>&$item) {
				$item = apply_filters('wpml_translate_single_string', $item, 'Ads Listing System', 'The option #' . $key . ' of content field #' . $this->id);
			}
		}
	}
}
		...
		if ($sitepress->get_default_language() != ICL_LANGUAGE_CODE) {
			if (isset($_GET['action']) && $_GET['action'] == 'configure' && isset($_GET['field_id'])) {
				foreach ($insert_update_args['selection_items'] AS $key=>$item) {
					...
					if ($option_string_id = icl_st_is_registered_string('Ads Listing System', 'The option #' . $key . ' of content field #' . $content_field_id))
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() == ICL_LANGUAGE_CODE) {
			foreach ($selection_items AS $key=>&$item) {
				do_action('wpml_register_single_string', 'Ads Listing System',  'The option #' . $key . ' of content field #' . $content_field->id, $item);
			}
		}
	}
}
