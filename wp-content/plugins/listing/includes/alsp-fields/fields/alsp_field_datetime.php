...
class alsp_content_field_datetime extends alsp_content_field {
	...
	public function configure() {
		...
		if (alsp_getValue($_POST, 'submit') && wp_verify_nonce($_POST['alsp_configure_content_fields_nonce'], ALSP_PATH)) {
			...
			if ($validation->run()) {
				...
				alsp_renderTemplate('alsp_fields/fields/alsp_datetime_configuration.tpl.php', array('content_field' => $this));
			}
		} else
			alsp_renderTemplate('alsp_fields/fields/alsp_datetime_configuration.tpl.php', array('content_field' => $this));
	}
	...
	public function renderInput() {
		...
		if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_datetime_input_'.$this->id.'.tpl.php'))) {
			$template = 'alsp_fields/fields/alsp_datetime_input.tpl.php';
		}
		
		$template = apply_filters('alsp_content_field_input_template', $template, $this);
			
		alsp_renderTemplate($template, array('content_field' => $this, 'dateformat' => alsp_getDatePickerFormat()));
	}
	
	public function validateValues(&$errors, $data) {
		$field_index_date_start = 'alsp-field-input-' . $this->id . '-start';
		$field_index_date_end = 'alsp-field-input-' . $this->id . '-end';
	...
	public function renderOutput($listing = null) {
		if ($this->value['date_start'] || $this->value['date_end']) {
			...
			if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_datetime_output_'.$this->id.'.tpl.php'))) {
				$template = 'alsp_fields/fields/alsp_datetime_output.tpl.php';
			}
			
			$template = apply_filters('alsp_content_field_output_template', $template, $this, $listing);
				
			alsp_renderTemplate($template, array('content_field' => $this, 'formatted_date_start' => $formatted_date_start, 'formatted_date_end' => $formatted_date_end, 'listing' => $listing));
		}
	}
	...
	public function renderOutputForMap($location, $listing) {
		if ($this->value['date']) {
			...
			return alsp_renderTemplate('alsp_fields/fields/alsp_datetime_output_map.tpl.php', array('content_field' => $this, 'formatted_date_start' => $formatted_date_start, 'formatted_date_end' => $formatted_date_end, 'listing' => $listing), true);
		}
	}
