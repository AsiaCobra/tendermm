...
class alsp_content_field_hours extends alsp_content_field {
	...
	public function configure() {
		...
		if (alsp_getValue($_POST, 'submit') && wp_verify_nonce($_POST['alsp_configure_content_fields_nonce'], ALSP_PATH)) {
			...
			if ($validation->run()) {
				...
				alsp_renderTemplate('alsp_fields/fields/alsp_hours_configuration.tpl.php', array('content_field' => $this));
			}
		} else
			alsp_renderTemplate('alsp_fields/fields/alsp_hours_configuration.tpl.php', array('content_field' => $this));
	}
	...
	public function renderInput() {
		...
		if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_hours_input_'.$this->id.'.tpl.php'))) {
			$template = 'alsp_fields/fields/alsp_hours_input.tpl.php';
		}
		
		$template = apply_filters('alsp_content_field_input_template', $template, $this);
			
		alsp_renderTemplate($template, array('content_field' => $this, 'week_days' => $week_days));
	}
	...
	public function renderOutput($listing = null) {
		if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_hours_output_'.$this->id.'.tpl.php'))) {
			$template = 'alsp_fields/fields/alsp_hours_output.tpl.php';
		}
	...
	public function processStrings() {
		...
		foreach ($week_days AS $key=>$day) {
			if ($this->value[$day.'_from'] || $this->value[$day.'_to'] || $this->value[$day.'_closed'])
				$strings[] = '<strong>' . $this->week_days_names[$key] . '</strong> ' . (($this->value[$day.'_closed']) ? __('Closed', 'ALSP') : $this->value[$day.'_from'] . ' - ' . $this->value[$day.'_to']);
		}
