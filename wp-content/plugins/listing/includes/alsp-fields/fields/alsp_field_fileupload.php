...
class alsp_content_field_fileupload extends alsp_content_field {
	...
	public function configure() {
		...
		if (alsp_getValue($_POST, 'submit') && wp_verify_nonce($_POST['alsp_configure_content_fields_nonce'], ALSP_PATH)) {
			...
			if ($validation->run()) {
				...
				alsp_renderTemplate('alsp_fields/fields/alsp_fileupload_configuration.tpl.php', array('content_field' => $this));
	...
	public function renderInput() {
		...
		if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_fileupload_input_'.$this->id.'.tpl.php'))) {

			$template = 'alsp_fields/fields/alsp_fileupload_input.tpl.php';
		...
		alsp_renderTemplate($template, array('content_field' => $this, 'file' => $file));
	...
	public function validateValues(&$errors, $data) {
		...
		if (!empty($_POST[$field_index_reset]) && (empty($_FILES) || !isset($_FILES[$field_index_input]) || !$_FILES[$field_index_input]['tmp_name'])) {
			...
			return array('id' => 0, 'text' => '', 'filess'=> $_FILES);
		...
		if ($this->canBeRequired() && $this->is_required) {
			...
			return array('id' => 0, 'text' => '', 'files'=> $_FILES);
	...
	public function renderOutput($listing = null) {
		...
		if (!($template = alsp_isTemplate('alsp_fields/fields/alsp_fileupload_output_'.$this->id.'.tpl.php'))) {

			$template = 'alsp_fields/fields/alsp_fileupload_output.tpl.php';
		...
		alsp_renderTemplate($template, array('content_field' => $this, 'listing' => $listing, 'file' => $file));
	...
	public function renderOutputForMap($location, $listing) {
		...
		return alsp_renderTemplate('alsp_fields/fields/alsp_fileupload_output_map.tpl.php', array('content_field' => $this, 'listing' => $listing, 'file' => $file), true);
