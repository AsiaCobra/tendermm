...
class alsp_content_fields_manager {
	...
	public function showContentFieldsTable() {
		...
		if (isset($_POST['submit_table'])) {
			if (isset($_POST['content_fields_order']) && $_POST['content_fields_order']) {
				if ($content_fields->saveOrder())
					alsp_addMessage(__('Content fields order were updated!', 'ALSP'), 'updated');
			}
			if ($content_fields->saveGroupsRelations())
				alsp_addMessage(__('Content fields relations were updated!', 'ALSP'), 'updated');
		}
		...
		alsp_renderTemplate('alsp_fields/alsp_fields_table.tpl.php', array('content_fields_table' => $content_fields_table, 'content_fields_groups_table' => $content_fields_groups_table, 'fields_types_names' => $content_fields->fields_types_names));
	}
			...
			if ($validation->run()) {
				if ($content_field->id) {
					...
					if ($content_fields->createContentFieldFromArray($validation->result_array())) {
						alsp_addMessage(__('Content field was created successfully!', 'ALSP'));
					}
				}
				$this->showContentFieldsTable();
				//wp_redirect(admin_url('admin.php?page=alsp_content_fields'));
				//die();
			} else {
				$content_field->buildContentFieldFromArray($validation->result_array());
				alsp_addMessage($validation->error_array(), 'error');
	
				alsp_renderTemplate('alsp_fields/alsp_add_edit_field.tpl.php', array('content_fields' => $content_fields, 'content_field' => $content_field, 'field_id' => $field_id, 'fields_types_names' => $content_fields->fields_types_names));
			}
		} else {
			alsp_renderTemplate('alsp_fields/alsp_add_edit_field.tpl.php', array('content_fields' => $content_fields, 'content_field' => $content_field, 'field_id' => $field_id, 'fields_types_names' => $content_fields->fields_types_names));
		}
			...
			if ($validation->run()) {
				if ($content_fields_group->id) {
					...
					if ($content_fields->createContentFieldsGroupFromArray($validation->result_array())) {
						alsp_addMessage(__('Content fields group was created successfully!', 'ALSP'));
					}
				}
				$this->showContentFieldsTable();
				//wp_redirect(admin_url('admin.php?page=alsp_content_fields'));
				...
				alsp_renderTemplate('alsp_fields/alsp_add_edit_fields_group.tpl.php', array('content_fields' => $content_fields, 'content_fields_group' => $content_fields_group, 'group_id' => $group_id));
			}
		} else {
			alsp_renderTemplate('alsp_fields/alsp_add_edit_fields_group.tpl.php', array('content_fields' => $content_fields, 'content_fields_group' => $content_fields_group, 'group_id' => $group_id));
		}
	...
	public function deleteContentField($field_id) {
		...
		if (($content_field = $content_fields->getContentFieldById($field_id)) && !$content_field->is_core_field) {
			if (alsp_getValue($_POST, 'submit')) {
				...
				alsp_renderTemplate('views/alsp_d_q.tpl.php', array('heading' => __('Delete field', 'ALSP'), 'question' => sprintf(__('Are you sure you want delete "%s" field?', 'ALSP'), $content_field->name), 'item_name' => $content_field->name));
		} else
			$this->showContentFieldsTable();
	}
		...
		if ($content_fields_group = $content_fields->getContentFieldsGroupById($group_id)) {
			if (alsp_getValue($_POST, 'submit')) {
				if ($content_fields->deleteContentFieldsGroup($group_id))
					alsp_addMessage(__('Fields group was deleted successfully!', 'ALSP'));
	
				$this->showContentFieldsTable();
				//wp_redirect(admin_url('admin.php?page=alsp_content_fields'));
				//die();
			} else
				alsp_renderTemplate('views/alsp_d_q.tpl.php', array('heading' => __('Delete fields group', 'ALSP'), 'question' => sprintf(__('Are you sure you want delete "%s" fields group?', 'ALSP'), $content_fields_group->name), 'item_name' => $content_fields_group->name));
		} else
			$this->showContentFieldsTable();
	}
	...
	public function contentFieldsMetabox($post) {
		...
		$content_fields = apply_filters('alsp_content_fields_metabox', $content_fields, $post);
		
		alsp_renderTemplate('alsp_fields/alsp_fields_metabox.tpl.php', array('content_fields' => $content_fields));
	}
...
class alsp_manage_content_fields_table extends WP_List_Table {
	...
	public function getItems($content_fields_object) {
		...
		foreach ($content_fields_object->content_fields_array as $id=>$content_field) {
			$items_array[$id] = array(
					...
					'on_exerpt_page_list' => $content_field->on_exerpt_page_list,
	...
	public function column_icon_image($item) {
		if ($item['icon_image'])
			return '<span class="alsp-icon-tag alsp-fa ' . $item['icon_image'] . '"></span>';
		else
			return ' ';
	}
	...
	public function column_in_pages($item) {
		...
		if ($item['on_exerpt_page'])
			$html[] = __('On Grid', 'ALSP');
		if ($item['on_exerpt_page_list'])
			$html[] = __('On List', 'ALSP');
