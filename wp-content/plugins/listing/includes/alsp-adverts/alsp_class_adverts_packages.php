...
class alsp_listings_packages {
	...
	public function __construct() {
		...
		add_action('alsp_listing_package_process_activate', array($this, 'listing_activate_package'));
	...
	public function available_listings_descr($level_id, $action_string, $directory = false) {
		...
		if ($number) {
			$out = sprintf(__("You have <strong>%s</strong> %s to %s from your package.", "ALSP"), $number, _n($directory->single, $directory->plural, $number), $action_string);
		}
	...
	public function submitlisting_level_message($level, $directory = false) {
		$out = $this->available_listings_descr($level->id, __("submit", "ALSP"), $directory);
