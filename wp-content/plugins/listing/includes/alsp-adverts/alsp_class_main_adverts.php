...
class alsp_listings_manager {
	...
	public function __construct() {
		...
		if ((isset($_POST['publish']) || isset($_POST['save']) || isset($_POST['alsp_save_as_active'])) && (isset($_POST['post_type']) && $_POST['post_type'] == ALSP_POST_TYPE)) {
	...
	public function listingInfoMetabox($post) {
		...
		alsp_renderTemplate('alsp-adverts/info_metabox.tpl.php', array('listing' => $listing, 'levels' => $levels));
	}
	...
	public function listingExpirationDateMetabox($post) {
		...
		if ($listing->status != 'expired') {
			...
			alsp_renderTemplate('alsp-adverts/change_date_metabox.tpl.php', array('listing' => $listing, 'dateformat' => alsp_getDatePickerFormat()));
		} else {
			...
			if ($ALSP_ADIMN_SETTINGS['alsp_fsubmit_addon'] && isset($alsp_instance->dashboard_page_url) && $alsp_instance->dashboard_page_url)
				echo '<br /><a href="' . alsp_dashboardUrl(array('alsp_action' => 'renew_listing', 'listing_id' => $listing->post->ID)) . '"><span class="alsp-field-icon pacz-icon-refresh"></span>' . $renew_link . '</a>';
			else
				echo '<br /><a href="' . admin_url('options.php?page=alsp_renew&listing_id=' . $listing->post->ID) . '"><span class="alsp-field-icon pacz-icon-refresh"></span>' . $renew_link . '</a>';
		}
	}
	...
	public function listingClaimMetabox($post) {
		...
		alsp_renderTemplate('alsp-adverts/claim_metabox.tpl.php', array('listing' => $listing));
	}

	public function listingContactEmailMetabox($post) {
		$listing = alsp_getCurrentListingInAdmin();

		alsp_renderTemplate('alsp-adverts/contact_email_metabox.tpl.php', array('listing' => $listing));
	}
	
	public function listingResurvaMetabox($post) {
		$listing = alsp_getCurrentListingInAdmin();

		alsp_renderTemplate('alsp-adverts/resurva_booking.tpl.php', array('listing' => $listing));
	}
	
	public function add_listings_table_columns($columns) {
		global $alsp_instance, $ALSP_ADIMN_SETTINGS;
		//$alsp_columns['alsp_level'] = __('Level', 'ALSP'); // needs workaround
		$alsp_columns['alsp_level'] = __('Level', 'ALSP') . (($alsp_instance->directories->isMultiDirectory()) ? '/' . __('Directory', 'ALSP') : '');
		...
		$alsp_columns['alsp_listing_id'] = __('ID', 'ALSP');
	...
	public function manage_listings_table_rows($column, $post_id) {
		...
		switch ($column) {
			case "alsp_level":
				...
				if ($listing->level && $listing->level->isUpgradable())
					echo ' <span class="alsp-field-icon pacz-li-settings"></span></s>';

				if ($listing->level && !$listing->level->eternal_active_period)
					echo '<br />(' . $listing->level->getActivePeriodString() . ')';
				
				if ($alsp_instance->directories->isMultiDirectory()) // needs workaround
			...
			case "alsp_expiration_date":
				...
				if ($listing->level && $listing->level->eternal_active_period)
					...
					if ($listing->status == 'expired') {
						...
						echo '<br /><a href="' . admin_url('options.php?page=alsp_renew&listing_id=' . $post_id) . '"><span class="alsp-field-icon pacz-icon-refresh"></span>' . $renew_link . '</a>';
	...
	public function add_row_actions($actions, $post) {
		if ($post->post_type == ALSP_POST_TYPE){
			...
			if ($listing->level->raiseup_enabled && $listing->status == 'active' && $listing->post->post_status == 'publish' && alsp_current_user_can_edit_advert($listing->post->ID)) {
				$raise_up_link = apply_filters('alsp_raiseup_option', __('raise up listing', 'ALSP'), $listing);
				$actions['raise_up'] = '<a href="' . admin_url('options.php?page=alsp_raise_up&listing_id=' . $post->ID) . '"><span class="pacz-icon-arrow-circle-o-up"></span>' . $raise_up_link . '</a>';
			}
	...
	public function posts_filter_dropdown() {
		...
		echo '<select name="alsp_post_status_filter">';
		echo '<option value="">' . __('Any post status', 'ALSP') . '</option>';
		echo '<option ' . selected(alsp_getValue($_GET, 'alsp_post_status_filter'), 'publish', false ) . 'value="publish">' . __('Published', 'ALSP') . '</option>';
		echo '<option ' . selected(alsp_getValue($_GET, 'alsp_post_status_filter'), 'pending', false ) . 'value="pending">' . __('Pending', 'ALSP') . '</option>';
		echo '<option ' . selected(alsp_getValue($_GET, 'alsp_post_status_filter'), 'draft', false ) . 'value="draft">' . __('Draft', 'ALSP') . '</option>';
		echo '</select>';
		
		echo '<select name="alsp_listing_status_filter">';
		echo '<option value="">' . __('Any listing status', 'ALSP') . '</option>';
		echo '<option ' . selected(alsp_getValue($_GET, 'alsp_listing_status_filter'), 'active', false ) . 'value="active">' . __('Active', 'ALSP') . '</option>';
		echo '<option ' . selected(alsp_getValue($_GET, 'alsp_listing_status_filter'), 'expired', false ) . 'value="expired">' . __('Expired', 'ALSP') . '</option>';
		echo '<option ' . selected(alsp_getValue($_GET, 'alsp_listing_status_filter'), 'unpaid', false ) . 'value="unpaid">' . __('Unpaid', 'ALSP') . '</option>';
		echo '</select>';
		
		// needs workaround
		if ($alsp_instance->directories->isMultiDirectory()) {
			echo '<select name="alsp_directory_filter">';
			echo '<option value="">' . __('All directories', 'ALSP') . '</option>';
			foreach ($alsp_instance->directories->directories_array AS $directory)
				echo '<option ' . selected(alsp_getValue($_GET, 'alsp_directory_filter'), $directory->id, false ) . 'value="' . $directory->id . '">' . $directory->name . '</option>';
			echo '</select>';
		}
	...
	public function posts_filter($vars) {
		global $ALSP_ADIMN_SETTINGS;
		if (isset($_GET['alsp_post_status_filter']) && $_GET['alsp_post_status_filter']) {
			$vars = array_merge(
				$vars,
				array(
						'post_status' => $_GET['alsp_post_status_filter']
				)
			);
		}
		if (isset($_GET['alsp_listing_status_filter']) && $_GET['alsp_listing_status_filter']) {
			$vars = array_merge(
				$vars,
				array(
						'meta_query' => array(
								...
								array(
										'key'     => '_listing_status',
										'value'   => $_GET['alsp_listing_status_filter'],
								)
						)
				)
			);
		}
		if (isset($_GET['alsp_directory_filter']) && $_GET['alsp_directory_filter']) {
			$vars = alsp_set_directory_args($vars, array($_GET['alsp_directory_filter']));
		}
	...
	public function raiseUpListing() {
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && alsp_current_user_can_edit_advert($listing_id)) {
			if ($this->loadCurrentListing($listing_id) && $this->current_listing->status == 'active') {
				...
				alsp_renderTemplate('alsp-adverts/raise_up.tpl.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action));
			} else
				exit();
		} else
			exit();
	...
	public function renewListing() {
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && alsp_current_user_can_edit_advert($listing_id)) {
			if ($this->loadCurrentListing($listing_id)) {
				...
				alsp_renderTemplate('alsp-adverts/renew.tpl.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action));
			} else
				exit();
		} else
			exit();
	...
	public function changeDateListingPage() {
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && alsp_current_user_can_edit_advert($listing_id)) {
			if ($this->loadCurrentListing($listing_id)) {
				...
				alsp_renderTemplate('alsp-adverts/change_date.tpl.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action, 'dateformat' => alsp_getDatePickerFormat()));
			} else
				exit();
		} else
			exit();
	...
	public function changeExpirationDate() {
		...
		if ($alsp_form_validation->run()) {
			...
			if ($this->current_listing->saveExpirationDate($alsp_form_validation->result_array()) && get_post_meta($this->current_listing->post->ID, '_listing_created', true)) {
		...
		} elseif ($error_string = $alsp_form_validation->error_array())
	...
	public function upgradeListingPage() {
		...
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && alsp_current_user_can_edit_advert($listing_id)) {
			if ($this->loadCurrentListing($listing_id)) {
				...
				alsp_renderTemplate('alsp-adverts/upgrade.tpl.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action, 'levels' => $alsp_instance->levels));
			} else
				exit();
		} else
			exit();
			...
			if (isset($_GET['upgrade_action']) && $_GET['upgrade_action'] == 'upgrade') {
				...
				if ($alsp_form_validation->run()) {
					...
					foreach ($listings_ids AS $listing_id) {
						if (is_numeric($listing_id) && alsp_current_user_can_edit_advert($listing_id))
							if ($this->loadCurrentListing($listing_id)) {
					...
					if ($upgraded)
						alsp_addMessage(sprintf(_n('%d listing has changed level successfully!', '%d listings have changed levels successfully!', $upgraded, 'ALSP'), $upgraded));
				} else
					exit();
			}

			alsp_renderTemplate('alsp-adverts/upgrade_bulk.tpl.php', array('listings_ids' => $listings_ids, 'referer' => $referer, 'action' => $action, 'levels' => $alsp_instance->levels));
		} else
			exit();
	}
	...
	public function processClaim() {
		...
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && alsp_current_user_can_edit_advert($listing_id)) {
			if ($this->loadCurrentListing($listing_id)) {
				...
				alsp_renderTemplate('alsp-adverts/claim_process.tpl.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action));
			} else
				exit();
		} else
			exit();
	...
	public function authornotetoAdmin() {
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && is_admin()) {
			if ($this->loadCurrentListing($listing_id)) {
				$referer = wp_get_referer();
				$action = 'show';
				alsp_renderTemplate('alsp-adverts/author_notice_to_admin.tpl.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action));
			}
		}
	}
	...
	public function saveInitialDraft($post_id) {
		...
		// $alsp_instance::setCurrentDirectory() for the frontend or self::setCurrentDirectory() for the backend
		if ($alsp_instance->current_directory) {
			add_post_meta($post_id, '_directory_id', $alsp_instance->current_directory->id);
		}

		$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->alsp_levels_relationships} (post_id, level_id) VALUES(%d, %d) ON DUPLICATE KEY UPDATE level_id=%d", $this->current_listing->post->ID, $this->current_listing->level->id, $this->current_listing->level->id));
	...
	public function saveDirectoryMeta($maybe_empty, $postarr) {
		...
		if (
			alsp_getValue($postarr, 'ID') &&
			alsp_getValue($postarr, 'post_type') == ALSP_POST_TYPE &&
			isset($_POST['directory_id']) &&
			is_numeric($_POST['directory_id']) &&
			($directory = $alsp_instance->directories->getDirectoryById($_POST['directory_id']))
		) {
			...
			if ($this->current_listing->directory->id != $directory->id) {
				...
				alsp_addMessage(__("Listing directory was changed!", "ALSP"));
			}
		}
	...
	public function validateListing($data, $postarr) {
		...
		if ($data['post_type'] == ALSP_POST_TYPE) {
			...
			if ($this->current_listing->level->locations_number) {
				if ($validation_results = $alsp_instance->locations_manager->validateLocations($this->current_listing->level, $errors)) {
			...
			if ($errors) {
				...
				alsp_addMessage(__('Listing was saved successfully!', 'ALSP'));
			}
		}
		return $data;
	}
	...
	public function setCurrentDirectory() {
		...
		if (is_admin() && $pagenow == 'post-new.php' && isset($_GET['post_type']) && $_GET['post_type'] == ALSP_POST_TYPE && isset($_GET['directory_id']) && is_numeric($_GET['directory_id']) && ($directory = $alsp_instance->directories->getDirectoryById($_GET['directory_id']))) {
			$alsp_instance->current_directory = $directory;
		}
	}
