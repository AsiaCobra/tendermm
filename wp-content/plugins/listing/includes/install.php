...
function alsp_install_directory() {
	...
	if (!get_option('alsp_installed_directory')) {
		...
		if (!$wpdb->get_var("SELECT id FROM {$wpdb->alsp_content_fields} WHERE slug = 'categories_list'"))
			$wpdb->query("INSERT INTO {$wpdb->alsp_content_fields} (`is_core_field`, `order_num`, `name`, `slug`, `description`, `type`, `icon_image`, `is_required`, `is_configuration_page`, `is_search_configuration_page`, `is_ordered`, `is_hide_name`, `is_hide_name_on_grid`, `is_hide_name_on_list`, `is_hide_name_on_search`, `is_field_in_line`, `on_exerpt_page`, `on_exerpt_page_list`, `on_listing_page`, `on_search_form`, `on_map`, `advanced_search_form`, `categories`, `options`, `search_options`, `group_id`) VALUES(1, 4, 'Categories', 'categories_list', '', 'categories', '', 0, 0, 0, 0, 0, 'hide', 'hide', 0, 0, 0, 0, 0, 0, 0, 0, '', '', '', '0');");
		if (!$wpdb->get_var("SELECT id FROM {$wpdb->alsp_content_fields} WHERE slug = 'listingtype_list'"))
		...
		if (!$wpdb->get_var("SELECT id FROM {$wpdb->alsp_content_fields} WHERE slug = 'listing_tags'"))
			$wpdb->query("INSERT INTO {$wpdb->alsp_content_fields} (`is_core_field`, `order_num`, `name`, `slug`, `description`, `type`, `icon_image`, `is_required`, `is_configuration_page`, `is_search_configuration_page`, `is_ordered`, `is_hide_name`, `is_hide_name_on_grid`, `is_hide_name_on_list`, `is_hide_name_on_search`, `is_field_in_line`, `on_exerpt_page`, `on_exerpt_page_list`, `on_listing_page`, `on_search_form`, `on_map`, `advanced_search_form`, `categories`, `options`, `search_options`, `group_id`) VALUES(1, 5, 'Listing Tags', 'listing_tags', '', 'tags', '', 0, 0, 0, 0, 0, 'hide', 'hide', 0, 0, 0, 0, 0, 0, 0, 0, '', '', '', '0');");
	//	if (!$wpdb->get_var("SELECT id FROM {$wpdb->alsp_content_fields} WHERE slug = 'price'"))
		//	$wpdb->query("INSERT INTO {$wpdb->alsp_content_fields} (`is_core_field`, `order_num`, `name`, `slug`, `description`, `type`, `icon_image`, `is_required`, `is_configuration_page`, `is_search_configuration_page`, `is_ordered`, `is_hide_name`, `is_hide_name_on_grid`, `is_hide_name_on_list`, `is_hide_name_on_search`, `is_field_in_line`, `on_exerpt_page`, `on_exerpt_page_list`, `on_listing_page`, `on_search_form`, `on_map`, `advanced_search_form`, `categories`, `options`, `search_options`, `group_id`) VALUES(0, 2, 'Price', 'price', '', 'price', '', 1, 1, 1, 0, 0, 'hide', 'hide', 0, 0, 1, 1, 1, 1, 1, 0, '', '', '', '1');");
		
		
		$wpdb->query("CREATE TABLE IF NOT EXISTS {$wpdb->alsp_directories} (
					`id` int(11) NOT NULL AUTO_INCREMENT,
					`name` varchar(255) NOT NULL,
					...
					`listingtype_slug` varchar(255) NOT NULL,
					`location_slug` varchar(255) NOT NULL,
					`tag_slug` varchar(255) NOT NULL,
					`categories` text NOT NULL,
					`listingtypes` text NOT NULL,
					`locations` text NOT NULL,
					`levels` text NOT NULL,
					PRIMARY KEY (`id`)
					) $collate ;");
		
		if (!$wpdb->get_var("SELECT id FROM {$wpdb->alsp_directories} WHERE name = 'Listings'"))
			$wpdb->query("INSERT INTO {$wpdb->alsp_directories} (`name`, `single`, `plural`, `listing_slug`, `category_slug`, `listingtype_slug`, `location_slug`, `tag_slug`, `categories`, `listingtypes`, `locations`, `levels`) VALUES ('Listings', 'listing', 'listings', 'business-listing', 'business-category', 'business-place', 'business-tag', '', '', '')");
		...
		$wpdb->query("CREATE TABLE IF NOT EXISTS {$wpdb->alsp_levels} (
					...
					`listingtype_number` tinyint(1) NOT NULL,
					`unlimited_categories` tinyint(1) NOT NULL,
					`unlimited_listingtype` tinyint(1) NOT NULL,
		...
		add_option('fix_2_0', 'fixed');
		add_option('fix_2_0_1', 'fixed');
		add_option('fix_2_0_2', 'fixed');
	}
