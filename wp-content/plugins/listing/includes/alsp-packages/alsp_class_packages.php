...
class alsp_levels {
	...
	public function createLevelFromArray($array) {
		...
		$insert_update_args = array(
				...
				'listingtype_number' => alsp_getValue($array, 'listingtype_number', 0),
				'locations_number' => alsp_getValue($array, 'locations_number', 1),
				'unlimited_categories' => alsp_getValue($array, 'unlimited_categories'),
				'unlimited_listingtype' => alsp_getValue($array, 'unlimited_listingtype'),
				'map' => alsp_getValue($array, 'map'),
				...
				'categories' => serialize($array['categories']),
				'listingtypes' => serialize($array['listingtypes']),
				'content_fields' => serialize(alsp_getValue($array, 'content_fields', array())),
				'locations_number' => alsp_getValue($array, 'locations_number', 1),
				'map_markers' => alsp_getValue($array, 'map_markers', 1),
		);
	...
	public function saveLevelFromArray($level_id, $array) {
		...
		$insert_update_args = array(
				...
				'listingtype_number' => alsp_getValue($array, 'listingtype_number'),
				'locations_number' => alsp_getValue($array, 'locations_number', 1),
				'unlimited_categories' => alsp_getValue($array, 'unlimited_categories'),
				'unlimited_listingtype' => alsp_getValue($array, 'unlimited_listingtype'),
				'map' => alsp_getValue($array, 'map'),
				...
				'listingtypes' => serialize($array['listingtypes']),
				'content_fields' => serialize(alsp_getValue($array, 'content_fields', array())),
				'locations' => serialize($array['locations']),
				'locations_number' => alsp_getValue($array, 'locations_number', 1),
				'map_markers' => alsp_getValue($array, 'map_markers', 1),
		);
		$insert_update_args = apply_filters('alsp_level_create_edit_args', $insert_update_args, $array);
...
class alsp_level {
	...
	public $listingtype_number = 0;
	public $unlimited_categories = 1;
	public $unlimited_listingtype = 1;
	...
	public function buildLevelFromArray($array) {
		...
		$this->listingtype_number = alsp_getValue($array, 'listingtype_number');
		$this->unlimited_categories = alsp_getValue($array, 'unlimited_categories');
		$this->unlimited_listingtype = alsp_getValue($array, 'unlimited_listingtype');
		$this->locations_number = alsp_getValue($array, 'locations_number');
		$this->map = alsp_getValue($array, 'map');
		$this->map_markers = alsp_getValue($array, 'map_markers');
	...
	public function convertListingTypes() {
		if ($this->listingtypes) {
			...
			if (!empty($unserialized_listingtypes)){
	...
	public function convertContentFields() {
		if ($this->content_fields) {
			...
			if (!empty($unserialized_content_fields)){
...
function alsp_levels_names_into_strings() {
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		foreach ($alsp_instance->levels->levels_array AS &$level) {
			$level->name = apply_filters('wpml_translate_single_string', $level->name, 'Ads Listing System', 'The name of level #' . $level->id);
			$level->description = apply_filters('wpml_translate_single_string', $level->description, 'Ads Listing System', 'The description of level #' . $level->id);
		}
	}
}
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() != ICL_LANGUAGE_CODE) {
			if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['level_id'])) {
				...
				if ($name_string_id = icl_st_is_registered_string('Ads Listing System', 'The name of level #' . $level_id))
					icl_add_string_translation($name_string_id, ICL_LANGUAGE_CODE, $insert_update_args['name'], ICL_TM_COMPLETE);
				if ($description_string_id = icl_st_is_registered_string('Ads Listing System', 'The description of level #' . $level_id))
...
function alsp_save_level_categories_locations($level_id, $array) {
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() != ICL_LANGUAGE_CODE) {
			update_option('alsp_wpml_level_categories_'.$level_id.'_'.ICL_LANGUAGE_CODE, alsp_getValue($array, 'categories'));
			update_option('alsp_wpml_level_listingtypes_'.$level_id.'_'.ICL_LANGUAGE_CODE, alsp_getValue($array, 'listingtypes'));
			update_option('alsp_wpml_level_locations_'.$level_id.'_'.ICL_LANGUAGE_CODE, alsp_getValue($array, 'locations'));
		}
		
		if ($sitepress->get_default_language() == ICL_LANGUAGE_CODE) {
			do_action('wpml_register_single_string', 'Ads Listing System', 'The name of level #' . $level_id, alsp_getValue($array, 'name'));
			do_action('wpml_register_single_string', 'Ads Listing System', 'The description of level #' . $level_id, alsp_getValue($array, 'description'));
		}
	}
}
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() != ICL_LANGUAGE_CODE) {
			foreach ($alsp_instance->levels->levels_array AS &$level) {
				...
				$_listingtypes = get_option('alsp_wpml_level_listingtypes_'.$level->id.'_'.ICL_LANGUAGE_CODE);
				if ($_listingtypes && (count($_listingtypes) > 1 || $_listingtypes != array('')))
					$level->listingtypes = $_listingtypes;
				else
					$level->listingtypes = array();
