...
class alsp_locationGeoname {
	...
	private function getURL($query) {
		...
		if (alsp_getMapEngine() == 'google') {
			...
			if (isset($ALSP_ADIMN_SETTINGS['alsp_address_autocomplete_code'])) {
				$iso3166 = strtolower($ALSP_ADIMN_SETTINGS['alsp_address_autocomplete_code']);
				if ($iso3166 == 'gb') {
					$iso3166 = 'uk';
				}
			...
			if (isset($ALSP_ADIMN_SETTINGS['alsp_address_autocomplete_code']) && !empty($ALSP_ADIMN_SETTINGS['alsp_address_autocomplete_code'])) {
				$iso3166 = strtolower($ALSP_ADIMN_SETTINGS['alsp_address_autocomplete_code']);
				if ($iso3166 == 'gb') {
					$iso3166 = 'uk';
				}
			...
			if ($ALSP_ADIMN_SETTINGS['alsp_mapbox_api_key']) {
				$fullUrl = sprintf("https://api.mapbox.com/geocoding/v5/mapbox.places/%s.json?language=en&access_token=%s%s", urlencode($query), $ALSP_ADIMN_SETTINGS['alsp_mapbox_api_key'], $country);
			}
