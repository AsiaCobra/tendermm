...
if( ! $total_announcements ) {
	echo '<div class="alert alert-warning alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="'.esc_attr__("close", "ALSP").'">&times;</a>'.apply_filters('difp_filter_announcement_empty', esc_html__("No announcements found.", 'ALSP') ).'</div>';
	return;
}
...
do_action('difp_display_before_announcementbox');
		...
		<div id="difp-table" class="difp-table difp-odd-even"><?php
			...
			?><div class="alert alert-warning"><?php _e('No announcements found. Try different filter.', 'ALSP'); ?></div><?php 
		}
		?></form><?php 
		wp_reset_postdata();