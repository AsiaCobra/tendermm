...
if( ! $total_bids ) {
	echo "<div class='difp-error'>".apply_filters('difp_filter_bixbox_empty', __("No offer found.", 'ALSP'), $action)."</div>";
	return;
}
...
do_action('difp_display_before_messagebox', $action);
		...
		<div id="difp-table" class="difp-table difp-odd-even row"><?php
			...
			?><div class="difp-error"><?php _e('No offer found. Try different filter.', 'ALSP'); ?></div><?php 
		}
		?></form><?php 
	wp_reset_postdata();
