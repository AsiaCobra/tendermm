...
function get_table_columns()
{
	$columns = array(
			...
			'thumb' => __('Thumbnail', 'ALSP'),
			'content' => __('Title', 'ALSP'),
	...
	switch( $column ) {
		...
		case 'content' :
			if( ! difp_is_read( true ) ) {
				$span = '<span class="difp-unread-classp"><span class="label label-danger">' .__("Unread", "ALSP"). '</span></span>';
				$class = ' difp-strong';
			} else {
				$span = '';
				$class = '';
			...
			if(isset($bid) && (!empty($bid))){
				$bid_wrap = '<span class="label label-success">'.esc_html__('Bid', 'ALSP').'</span>';
			}else{
				$bid_wrap = '';
			}
