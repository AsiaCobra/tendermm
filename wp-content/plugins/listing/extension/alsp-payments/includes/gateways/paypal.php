		...
		if ($paypal->validateIpn()) {
			if (strcasecmp($paypal->ipnData['business'], $ALSP_ADIMN_SETTINGS['alsp_paypal_email']) === 0) {
				if (is_unique_transaction($paypal->ipnData['txn_id'])) {
					...
					if (($invoice = getInvoiceByID($invoice_id)) && floatval($paypal->ipnData['mc_gross']) == $invoice->taxesPrice(false) && $paypal->ipnData['mc_currency'] == $ALSP_ADIMN_SETTINGS['alsp_payments_currency']) {
						if (alsp_create_transaction(
							...
							if ($paypal->ipnData['payment_status'] == 'Completed') {
								...
								$invoice->logMessage(sprintf(__('Payment status: %s', 'ALSP'),  $paypal->ipnData['payment_status']));
							}
						}
					}
				}
