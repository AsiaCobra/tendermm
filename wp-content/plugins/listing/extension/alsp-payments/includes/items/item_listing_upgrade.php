...
add_filter('alsp_listing_upgrade', 'alsp_create_upgrade_listing_invoice', 10, 3);
function alsp_create_upgrade_listing_invoice($continue, $listing, $continue_invoke_hooks) {
		...
		if (recalcPrice($listing->level->upgrade_meta[$new_level_id]['price']) > 0) {
			if (!($invoice_id = get_post_meta($listing->post->ID, '_listing_upgrade_invoice', true)) || !($invoice = getInvoiceByID($invoice_id))) {
				...
				if ($invoice_id = call_user_func_array('alsp_create_invoice', $invoice_args)) {
					alsp_addMessage(sprintf(__('New <a href="%s">invoice</a> was created successfully, listing "%s" will be upgraded after payment', 'ALSP'), alsp_get_edit_invoice_link($invoice_id, 'url'), $listing->title()));
