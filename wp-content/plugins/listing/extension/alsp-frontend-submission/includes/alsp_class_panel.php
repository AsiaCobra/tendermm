...
class alsp_dashboard_controller extends alsp_public_control {
	...
	public function init($args = array(), $args2 = array(), $args3 = array(), $args4 = array()) {
		...
		if (!is_user_logged_in()) {
			...
			if (!$alsp_instance->action) {
				...
				$this->template = array(ALSP_FSUBMIT_TEMPLATES_PATH, 'alsp_panel.tpl.php');

				$this->subtemplate = array(ALSP_FSUBMIT_TEMPLATES_PATH, 'adverts.tpl.php');

			} elseif ($alsp_instance->action == 'edit_advert' && $listing_id) {
				...
				if (alsp_current_user_can_edit_advert($listing_id)) {
					...
					if (isset($_POST['submit'])) {
						...
						if ($errors) {
							...
							if ($listing->post->post_status == 'publish') {
								...
								if ($ALSP_ADIMN_SETTINGS['alsp_fsubmit_edit_moderation']) {
							...
							if (is_wp_error($result)) {
								...
								if ($ALSP_ADIMN_SETTINGS['alsp_editlisting_admin_notification']) {
									...
									if ($listing->post->post_status == 'publish' && $post_status == 'pending') {
										...
										$subject = __('Notification about listing modification (do not reply)', 'ALSP');
										...
										$ALSP_ADIMN_SETTINGS['alsp_editlisting_admin_notification'])));
								...
								if (strpos($this->referer, alsp_dashboardUrl()) !== false) {
					...
					$this->subtemplate = array(ALSP_FSUBMIT_TEMPLATES_PATH, 'edit_advert.tpl.php');
			...
			} elseif ($alsp_instance->action == 'raiseup_listing' && $listing_id) {

				if (alsp_current_user_can_edit_advert($listing_id) && ($listing = alsp_getListing($listing_id)) && $listing->status == 'active') {
			...
			}elseif ($alsp_instance->action == 'renew_listing' && $listing_id) {

				if (alsp_current_user_can_edit_advert($listing_id) && ($listing = alsp_getListing($listing_id))) {
			...
			} elseif ($alsp_instance->action == 'delete_listing' && $listing_id) {
				...
				if ($listing_id && alsp_current_user_can_edit_advert($listing_id) && ($listing = $alsp_instance->listings_manager->loadListing($listing_id))) {
					...
					if (isset($_GET['status_action']) && $_GET['status_action'] == 'private') {
						...
						//if (wp_update_post($listing_id, true) !== FALSE) {
							...
							alsp_addMessage(__('Listing status was changed from "Public" to "Private" successfully!', 'ALSP'));
						...
						//if (wp_update_post($listing_id, true) !== FALSE) {
							...
							alsp_addMessage(__('Listing status was changed from "Private" to "Public" successfully!', 'ALSP'));
			...
			} elseif ($alsp_instance->action == 'upgrade_listing' && $listing_id) {
			...
			} elseif ($alsp_instance->action == 'view_stats' && $listing_id) {

				if ($ALSP_ADIMN_SETTINGS['alsp_enable_stats'] && alsp_current_user_can_edit_advert($listing_id) && ($listing = alsp_getListing($listing_id))) {
			...
			}elseif ($alsp_instance->action == 'claim_listing' && $listing_id) {

				if (($listing = alsp_getListing($listing_id)) && $listing->is_claimable) {
					...
					if ($listing->post->post_author != $claimer_id) {
							...
							if ($listing->claim->updateRecord($claimer_id, $claimer_message, 'pending')) {
								...
								if ($ALSP_ADIMN_SETTINGS['alsp_claim_approval']) {
									...
									if ($ALSP_ADIMN_SETTINGS['alsp_claim_notification']) {
										...
										$subject = __('Claim notification', 'ALSP');
										...
										$body = str_replace('[author]', $author->display_name,
												...
												str_replace('[link]', alsp_dashboardUrl(array('listing_id' => $listing->post->ID, 'alsp_action' => 'process_claim')),
						...
						if ($this->action == 'show') {
							...
							if ($ALSP_ADIMN_SETTINGS['alsp_claim_approval']) {
								...
								alsp_addMessage(__('Notification about claim for this listing will be sent to the current listing owner.', 'ALSP'));

								alsp_addMessage(__("After approval you will become owner of this listing, you'll receive email notification.", 'ALSP'));

							}

							if ($ALSP_ADIMN_SETTINGS['alsp_after_claim'] == 'expired') {

								alsp_addMessage(__('After approval listing status become expired.', 'ALSP') . (($ALSP_ADIMN_SETTINGS['alsp_payments_addon']) ? apply_filters('alsp_renew_option', __(' The price for renewal', 'ALSP'), $alsp_instance->current_listing) : ''));
			...
			}elseif ($alsp_instance->action == 'process_claim' && $listing_id) {

				if (alsp_current_user_can_edit_advert($listing_id) && ($listing = alsp_getListing($listing_id)) && $listing->claim->isClaimed()) {
			...
			// adapted for WPML
			...
			}  elseif (function_exists('wpml_object_id_filter') && $sitepress && $ALSP_ADIMN_SETTINGS['alsp_enable_frontend_translations'] && $alsp_instance->action == 'add_translation' && isset($_GET['listing_id']) && isset($_GET['to_lang'])) {
				...
				if ($sitepress->is_translated_post_type($post_type)) {
					...
					if ($new_listing_id = $iclTranslationManagement->make_duplicate($master_post_id, $lang_code)) {
						...
						wp_redirect(add_query_arg(array('alsp_action' => 'edit_advert', 'listing_id' => $new_listing_id), get_permalink(apply_filters('wpml_object_id', $alsp_instance->dashboard_page_id, 'page', true, $lang_code))));
			...
			$alsp_instance->listings_manager->loadCurrentListing($listing_id);

		}



		apply_filters('alsp_dashboard_controller_construct', $this);
	...
	public function enqueue_scripts_styles() {
		...
		wp_register_script('alsp_stats', ALSP_RESOURCES_URL . 'js/min/chart.min.js');
