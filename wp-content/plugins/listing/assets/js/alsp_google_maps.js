	...
	console.error = function() {
	    ...
	    if (typeof err == "string") {
		    ...
		    if (err.indexOf('RefererNotAllowedMapError') != -1) {
		    	...
		    	alert('Ads Listing System plugin: the current URL loading the Google Maps has not been added to the list of allowed referrers. Please check the "Accept requests from these HTTP referrers (web sites)" field in Google API console.*.'+hostname+'/*, '+protocol+'//'+hostname+'/*, '+protocol+'//www.'+hostname+'/*');
		    }
		    if (err.indexOf('ApiNotActivatedMapError') != -1) {
		    	alert('Ads Listing System plugin: you have to enable following APIs in Google API console https://code.google.com/apis/console : Google Maps JavaScript API, Google Static Maps API, Google Places API Web Service, Google Maps Geocoding API and Google Maps Directions API. Note, that it requires some time for changes to take effect.');
		    }
...
(function($) {
	...
	window.alsp_setMapZoomCenter = function(map_id, map_attrs, markers_array) {
	    ...
	    if (typeof map_attrs.start_latitude != 'undefined' && map_attrs.start_latitude && typeof map_attrs.start_longitude != 'undefined' && map_attrs.start_longitude) {
			...
			// no fitbounds here
			alsp_setMapCenter(map_id, new google.maps.LatLng(34, 0));
		    alsp_setMapZoom(map_id, zoom_level);
		}
	}
